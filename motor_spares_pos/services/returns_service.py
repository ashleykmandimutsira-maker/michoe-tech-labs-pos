...
class ReturnsService:
    ...
    def calculate_returnable_quantity(self, product_id: int, original_qty: int, 
                                     sale_id: int) -> int:
        """
    ...
    def create_return(self, invoice_number: str, items: List[dict], reason: str, 
                     user_id: int, customer_name: str = "", 
                     customer_phone: Optional[str] = None) -> Return:
        """
        ...
        for item_data in items:
            ...
            if not original_item:
                raise ValueError(f"Product not found in original sale: {product_id}")
        ...
        logger.info(f"Return created: {return_obj.return_number} for invoice {invoice_number}")
        return return_obj
            ...
            with self.db.transaction() as conn:
                ...
                for item in return_obj.items:
                    conn.execute(
                        ...
                        (return_obj.id, item.sale_item_id, item.product_id, item.quantity, item.unit_price,
                         item.vat_rate, item.line_total, item.condition, item.return_reason),
                    )
            
            logger.info(f"Return saved: {return_obj.return_number} (ID: {return_obj.id})")
            self.sync.enqueue("RETURN", return_obj.id, "CREATE", json.dumps(return_obj.to_dict()))
    ...
    def approve_return(self, return_id: int, approved_by_user_id: int) -> bool:
        ...
        """
        # Check authorization
        if not self.permission_service.has_permission(approved_by_user_id, 'APPROVE_RETURN'):
            raise PermissionError(f"User {approved_by_user_id} cannot approve returns")
    ...
    def process_return(self, return_id: int, user_id: int) -> bool:
        ...
        if return_obj.status != 'APPROVED':
            raise ValueError(f"Return must be approved before processing (status: {return_obj.status})")
        
        try:
            with self.db.transaction() as conn:
                for item in return_obj.items:
                    product = conn.execute("SELECT quantity_on_hand FROM products WHERE id=?", (item.product_id,)).fetchone()
                    if product is None:
                        raise ValueError(f"Product not found: {item.product_id}")
                    previous = int(product[0])
                    restored = item.condition in ('GOOD', 'WRONG_PART')
                    new_quantity = previous + item.quantity if restored else previous
                    ...
                    conn.execute(
                        ...
                        (item.product_id, 'RETURN' if restored else 'DAMAGE', item.quantity if restored else 0,
                         previous, new_quantity, f"RETURN-{return_id}", user_id, f"Returned {item.condition}: {item.return_reason or ''}"),
                    )
                conn.execute("UPDATE returns SET status='COMPLETED', updated_at=CURRENT_TIMESTAMP WHERE id=? AND status='APPROVED'", (return_id,))
    ...
    def process_refund(self, return_id: int, refund_method: str, user_id: int) -> int:
        """
        ...
        # Check permission
        if not self.permission_service.has_permission(user_id, 'PROCESS_REFUND'):
            raise PermissionError(f"User {user_id} cannot process refunds")
        
        # Check large refund threshold
        if return_obj.total_refund > self.RETURN_LIMITS['LARGE_REFUND_THRESHOLD_USD']:
            if not self.permission_service.has_permission(user_id, 'PROCESS_LARGE_REFUND'):
                raise PermissionError(
                    f"Large refund ({return_obj.total_refund}) requires PROCESS_LARGE_REFUND permission"
                )
        
        try:
            if self.db.execute_query("SELECT 1 FROM refunds WHERE return_id=? LIMIT 1", (return_id,)):
                raise ValueError(f"Refund already processed for return {return_id}")
            # Create refund record
            refund = Refund(
                return_id=return_id,
                refund_amount=return_obj.total_refund,
                refund_method=refund_method,
                currency={'CASH_USD': 'USD', 'CASH_ZIG': 'ZiG'}.get(refund_method, 'ZWL'),
                status='COMPLETED',
            ...
            logger.info(f"Refund processed: {refund.id} for return {return_id}")
            self.sync.enqueue("REFUND", refund.id, "CREATE", json.dumps(refund.to_dict()))
            self.sync.request_background_sync()
            return refund.id
        except Exception as e:
