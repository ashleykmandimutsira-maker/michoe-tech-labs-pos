...
class ProductService:
    ...
    def create_product(self, product: Product) -> int:
        ...
        if existing:
            raise ValueError(f"A product with part number '{product.part_no}' already exists")
        if product.barcode:
            existing_barcode = self.db.execute_query(
                "SELECT id FROM products WHERE barcode = ?", (product.barcode,)
            )
            if existing_barcode:
                raise ValueError(f"A product with barcode '{product.barcode}' already exists")
        ...
        try:
            ...
            logger.info(f"Created product: {product.part_no} (ID: {product_id})")
            created = self.get_product_by_id(product_id)
            if created:
                self.sync.enqueue("PRODUCT", product_id, "CREATE", json.dumps(created.to_dict()))
    ...
    def update_product(self, product: Product) -> bool:
        ...
        try:
            ...
            self.db.execute_update(query, params)
            logger.info(f"Updated product: {product.part_no} (ID: {product.id})")
            updated = self.get_product_by_id(product.id)
            if updated:
                if previous and previous.selling_price != updated.selling_price:
                    AuditService().log_price_change(product.id, None, previous.selling_price, updated.selling_price, "Product update")
                self.sync.enqueue("PRODUCT", product.id, "UPDATE", json.dumps(updated.to_dict()))
                self.sync.request_background_sync()
            return True
        except Exception as e:
    ...
    def archive_product(self, product_id: int, requesting_user_id: int,
        ...
        requesting_user_id = getattr(requesting_user_id, 'id', requesting_user_id)
        approving_admin_id = getattr(approving_admin_id, 'id', approving_admin_id)
        permissions = PermissionService()
        if permissions.can_archive_product(requesting_user_id):
            approver = requesting_user_id
        else:
            if not approving_admin_id or not permissions.can_archive_product(approving_admin_id):
                raise PermissionError("Product archive requires ADMIN authorization")
        ...
        if archived:
            self.sync.enqueue("PRODUCT", product_id, "UPDATE", json.dumps(archived.to_dict()))
            self.sync.request_background_sync()
        AuditService().log_product_archive(product_id, requesting_user_id, approver, reason or None)
        logger.info("Archived product %s by user %s (approved by %s)", product_id, requesting_user_id, approver)
        return True

    def archive_all_active_products(self, requesting_user_id: int, reason: str = "", request_sync: bool = True) -> int:
        ...
        if not permissions.can_archive_product(requesting_user_id):
            raise PermissionError("Only an authorized administrator can archive all products")
        with self.db.transaction() as conn:
            rows = conn.execute("SELECT * FROM products WHERE active=1 ORDER BY id").fetchall()
            if not rows:
                return 0
            conn.execute("UPDATE products SET active=0, updated_at=CURRENT_TIMESTAMP WHERE active=1")
            payloads = []
            for row in rows:
                payload = dict(row); payload['active'] = False
                payloads.append(("PRODUCT", row['id'], "UPDATE", json.dumps(payload), "PENDING"))
            conn.executemany(
            ...
            conn.execute(
                ...
                (requesting_user_id, requesting_user_id, reason or None, f"Archived {len(rows)} active products"),
            )
        if request_sync:
            self.sync.request_background_sync()
        emit_change("PRODUCT", {"operation": "BULK_ARCHIVE", "count": len(rows)})
        logger.info("Archived %s active products in one bulk operation", len(rows))
        return len(rows)
    ...
    def clear_active_catalog(self, requesting_user_id: int) -> dict:
        ...
        if not PermissionService().can_archive_product(requesting_user_id):
            raise PermissionError("Only an authorized administrator can clear the catalog")
        with self.db.transaction() as conn:
            rows = conn.execute("""
                SELECT p.*,
            ...
            if archived:
                conn.executemany("UPDATE products SET active=0,updated_at=CURRENT_TIMESTAMP WHERE id=?", [(row['id'],) for row in archived])
            if removable:
                conn.execute("DROP TRIGGER IF EXISTS prevent_product_physical_delete")
                conn.executemany("DELETE FROM products WHERE id=?", [(row['id'],) for row in removable])
                conn.executemany("INSERT OR REPLACE INTO product_tombstones(product_id,part_no) VALUES(?,?)", [(row['id'], row['part_no']) for row in removable])
            ...
            for row in archived:
                payload = dict(row); payload['active'] = False
                queued.append(('PRODUCT', row['id'], 'UPDATE', json.dumps(payload), 'PENDING'))
            for row in removable:
                queued.append(('PRODUCT', row['id'], 'DELETE', json.dumps({'id': row['id'], 'part_no': row['part_no'], 'deleted': True}), 'PENDING'))
            if queued:
                conn.executemany("""INSERT INTO sync_queue(entity_type,entity_id,operation,payload,status)
                    VALUES(?,?,?,?,?) ON CONFLICT(entity_type,entity_id,operation) DO UPDATE SET
                    payload=excluded.payload,status='PENDING',error_message=NULL,last_attempt=NULL""", queued)
            conn.execute("""INSERT INTO audit_logs(action,entity_type,requesting_user_id,approving_user_id,details,status)
                VALUES('PRODUCTS_CATALOG_CLEARED','PRODUCT',?,?,?,'COMPLETED')""", (requesting_user_id, requesting_user_id, f"Deleted {len(removable)} unreferenced products; archived {len(archived)} historical products"))
        self.sync.request_background_sync()
        emit_change('PRODUCT', {'operation': 'CATALOG_CLEAR', 'deleted': len(removable), 'archived': len(archived)})
        return {'deleted': len(removable), 'archived': len(archived)}

    def delete_product(self, product_id: int, requesting_user_id: int,
                       approving_admin_id: Optional[int] = None,
                       reason: str = "") -> bool:
        ...
        if permissions.can_archive_product(requesting_user_id):
            approver = requesting_user_id
        elif approving_admin_id and permissions.can_archive_product(approving_admin_id):
            approver = approving_admin_id
        else:
            raise PermissionError("Permanent deletion requires ADMIN authorization")
        product = self.get_product_by_id(product_id)
        if not product:
            raise ValueError(f"Product not found: {product_id}")
        ...
        for table in history_tables:
            if self.db.execute_query(f"SELECT 1 FROM {table} WHERE product_id=? LIMIT 1", (product_id,)):
                history.append(table)
        if history:
            raise ValueError("Product has historical records and cannot be permanently deleted; archive it instead: " + ', '.join(history))
        try:
            with self.db.transaction() as conn:
                conn.execute("DROP TRIGGER IF EXISTS prevent_product_physical_delete")
                ...
                conn.execute("""
                    ...
                    END
                """)
            AuditService().log_action('PRODUCT_PERMANENTLY_DELETED', 'PRODUCT', product_id, requesting_user_id, reason or None, f'Approved by: {approver}', approving_user_id=approver)
            self.sync.enqueue("PRODUCT", product_id, "DELETE", json.dumps({'id': product_id, 'part_no': product.part_no, 'deleted': True}))
            self.sync.request_background_sync()
            return True
        except Exception:
            logger.exception("Failed to permanently delete product %s", product_id)
            raise

    def restore_product(self, product_id: int, requesting_user_id: int,
                        approving_admin_id: Optional[int] = None,
                        reason: str = "") -> bool:
        ...
        if permissions.can_restore_product(requesting_user_id):
            ...
            if not approving_admin_id or not permissions.can_restore_product(approving_admin_id):
                raise PermissionError("Product restore requires ADMIN authorization")
            approver = approving_admin_id
        product = self.get_product_by_id(product_id)
        if not product:
            raise ValueError(f"Product not found: {product_id}")
        ...
        if restored:
            self.sync.enqueue("PRODUCT", product_id, "UPDATE", json.dumps(restored.to_dict()))
            self.sync.request_background_sync()
        AuditService().log_product_restore(product_id, requesting_user_id, approver, reason or None)
        logger.info("Restored product %s by user %s (approved by %s)", product_id, requesting_user_id, approver)
        return True
    ...
    def get_products_by_vehicle(self, vehicle_make: str, vehicle_model: str = "") -> List[Product]:
        """
        ...
        if vehicle_model:
            ...
            """
            results = self.db.execute_query(query, (vehicle_make, vehicle_model))
        else:
            query = """
                SELECT * FROM products
    ...
    def create_vehicle_model(self, vehicle: VehicleModel) -> int:
        ...
        query = """
            ...
            VALUES (?, ?, ?, ?)
        """
        params = (vehicle.make, vehicle.model, vehicle.year_from, vehicle.year_to)

        try:
            self.db.execute_update(query, params)
            vehicle_id = self.db.get_last_insert_id()
            logger.info(f"Created vehicle: {vehicle.make} {vehicle.model} (ID: {vehicle_id})")
            return vehicle_id
        except Exception as e:
    ...
    def _row_to_product(row) -> Product:
        ...
        return Product(
            ...
            barcode=row['barcode'],
            part_no=row['part_no'],
            oem_number=row['oem_number'] if 'oem_number' in row.keys() else None,
            description=row['description'],
            brand=row['brand'],
            category_id=row['category_id'],
            vehicle_make=row['vehicle_make'],
            vehicle_model=row['vehicle_model'],
            vehicle_year_from=row['vehicle_year_from'] if 'vehicle_year_from' in row.keys() else None,
            vehicle_year_to=row['vehicle_year_to'] if 'vehicle_year_to' in row.keys() else None,
