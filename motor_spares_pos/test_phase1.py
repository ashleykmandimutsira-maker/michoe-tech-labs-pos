...
def test_database_initialization():
    ...
    # Check the database file that this DatabaseManager actually opened
    # (not a hardcoded relative path, which may not match the real one).
    assert os.path.exists(db.db_path), f"Database file not found: {db.db_path}"
...
def test_product_operations():
    ...
    if product.barcode:
        ...
        assert found and found.id == product.id, f"Barcode search failed for {product.barcode}"
        logger.info(f"✓ Barcode search working: {product.barcode}")

    # Get by part number
    found = product_service.get_product_by_part_no(product.part_no)
    assert found and found.id == product.id, f"Part number search failed for {product.part_no}"
...
def test_stock_movements():
    ...
    try:
        if original_stock >= 2:
            ...
            assert updated_product.quantity_on_hand == original_stock - 2, (
    ...
    assert updated_product.quantity_on_hand == expected_stock, (
...
def main():
    ...
    def run_check(name, func):
        """Run a test function and record pass/fail from whether it raises,
        the same convention pytest itself uses for these functions now."""
        try:
            func()
            results[name] = True
        except Exception as e:
    ...
    # Run tests
    run_check('Database', test_database_initialization)
    
    if results['Database']:
        initialize_test_data()
        run_check('Products', test_product_operations)
        run_check('StockLevels', test_stock_levels)
        run_check('Movements', test_stock_movements)
