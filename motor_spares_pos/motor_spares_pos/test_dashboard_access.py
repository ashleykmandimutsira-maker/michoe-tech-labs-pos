...
class DashboardAccessTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        cashier_id = auth.create_user(
            "dash_cashier", "cash123", "Dashboard Cashier", 3
        )
    ...
    def test_cashier_cannot_see_or_route_to_dashboard(self):
        ...
        with patch.object(QMessageBox, "warning") as warning:
            ...
            warning.assert_called_once_with(
                window, "Access denied", "Access denied."
            )
        self.assertEqual("POS", window.header.text())
        window.close()

    def test_explicit_dashboard_permission_allows_dashboard(self):
        permission_id = self.auth.db.execute_query(
            "SELECT id FROM permissions WHERE code='dashboard.view'"
        )[0]["id"]
        self.auth.grant_permission(
            self.cashier_id, permission_id, self.admin.id
        )
