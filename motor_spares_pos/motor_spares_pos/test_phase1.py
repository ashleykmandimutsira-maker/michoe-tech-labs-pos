...
def initialize_test_data():
    ...
    for prod_data in test_products:
        try:
            ...
            logger.info(
                f"✓ Created product: {prod_data['part_no']} (ID: {product_id})"
            )
            created_count += 1
        except Exception as e:
            if "UNIQUE constraint failed" not in str(e):
                logger.warning(
                    f"✗ Failed to create product {prod_data['part_no']}: {e}"
                )
            else:
                logger.info(
                    f"✓ Product already exists: {prod_data['part_no']}"
                )
                created_count += 1
...
def test_stock_levels():
    ...
    try:
        ...
        for product in low_stock:
            logger.info(
                f"  - {product.part_no}: {product.quantity_on_hand}/{product.reorder_level}"
            )
        ...
        logger.info(
            f"  Inventory value: {summary['inventory_value']:,.2f} ZWL"
        )
...
def test_stock_movements():
    ...
    try:
        ...
        logger.info(
            f"Testing with product: {test_product.part_no} (current stock: {original_stock})"
        )
        ...
        try:
            if original_stock >= 2:
                ...
                updated_product = product_service.get_product_by_id(
                    test_product.id
                )
                if updated_product.quantity_on_hand == original_stock - 2:
                    logger.info(
                        f"✓ Stock updated correctly: {original_stock} -> {updated_product.quantity_on_hand}"
                    )
                else:
                    logger.error(
                        f"✗ Stock not updated correctly: expected {original_stock - 2}, got {updated_product.quantity_on_hand}"
                    )
            else:
                logger.info(
                    f"⚠ Skipping SALE test (insufficient stock: {original_stock})"
                )
        ...
        expected_stock = (
            original_stock - 2 if original_stock >= 2 else original_stock
        ) + 5
        if updated_product.quantity_on_hand == expected_stock:
            logger.info(f"✓ Stock increased correctly: {expected_stock}")
        else:
            logger.error(
                f"✗ Stock not increased correctly: expected {expected_stock}, got {updated_product.quantity_on_hand}"
            )

        # Get movement history
        history = inventory_service.get_product_movement_history(
            test_product.id, limit=10
        )
        logger.info(f"✓ Movement history: {len(history)} records")
        for movement in history[:3]:
            logger.info(
                f"  - {movement.movement_type}: qty={movement.quantity}, stock={movement.previous_quantity}->{movement.new_quantity}"
            )
...
def main():
    ...
    print(
        "█"
        + "  Motor Spares POS - PHASE 1 Command-Line Test Suite".center(68)
        + "█"
    )
    print("█" + " " * 68 + "█")
    print("█" * 70)
