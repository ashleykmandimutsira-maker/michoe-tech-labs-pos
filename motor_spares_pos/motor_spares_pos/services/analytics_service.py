...
class AnalyticsService:
    ...
    def _where(
        ...
        return (
            "s.created_at >= ? AND s.created_at < ? AND s.status = 'COMPLETED'",
            (begin, finish),
        )
    ...
    def get_sales_summary(
        ...
        sales_row = self.db.execute_query(
            f"SELECT COALESCE(SUM(s.total), 0) revenue, COUNT(*) transactions FROM sales s WHERE {where}",
            params,
        )[0]
        item_row = self.db.execute_query(
            f"""
        ...
        return {
            ...
            "average_transaction": (
                revenue / transactions if transactions else 0.0
            ),
            "returns": returns,
        }
    ...
    def get_sales_trend(
        ...
        where, params = self._where(period, start, end)
        metric_expr = {
            "profit": "SUM((si.unit_price - p.cost_price) * si.quantity)",
            "units_sold": "SUM(si.quantity)",
        }.get(metric)
        if period == "today":
        ...
        if metric in ("revenue", "transactions"):
            ...
            rows = self.db.execute_query(
                f"SELECT {label} label, {expr} value FROM sales s WHERE {where} GROUP BY {group} ORDER BY {group}",
                params,
            )
        else:
            rows = self.db.execute_query(
        ...
        return [
            {"label": row["label"] or "", "value": float(row["value"] or 0)}
            for row in rows
        ]
    ...
    def get_vehicle_sales(
        ...
        extra, extra_params = "", []
        if make:
            extra += " AND p.vehicle_make = ?"
            extra_params.append(make)
        if model:
            extra += " AND p.vehicle_model = ?"
    ...
    def get_return_summary(
        ...
        return {
            ...
            "top_products": [dict(x) for x in top],
        }

    def get_payment_method_sales(
        self,
        period: str = "year",
        start: Optional[str] = None,
        end: Optional[str] = None,
    ...
    def get_daily_sales_report(
        ...
        for row in rows:
            returns = self.db.execute_query(
                "SELECT COALESCE(SUM(total_refund),0) value FROM returns WHERE date(created_at)=? AND status != 'CANCELLED'",
                (row["day"],),
            )[0]["value"]
            item = dict(row)
            item["returns"] = float(returns or 0)
            item["net_sales"] = (
                float(item["gross_sales"] or 0) - item["returns"]
            )
            result.append(item)
        return result
