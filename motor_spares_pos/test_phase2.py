...
def _phase2_test_database():
    """Run this suite's own initialize_test_data() automatically under pytest.

    This file's run_all_tests() driver (used when run as a script) already
    called initialize_test_data() once before running any test. pytest
    instead discovers and calls each test_* function directly, so without
    this fixture that setup step never runs and every test falls back to
    whichever database the process-wide DatabaseManager singleton happens
    to still be holding -- typically a temp directory a different test
    module has already deleted. This keeps every test in this module
    sharing one continuous database/session, exactly like the original
    script-mode run, in its own file under data/ (never data/pos.db).
    """
