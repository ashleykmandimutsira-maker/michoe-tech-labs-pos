...
def initialize_test_data():
    ...
    if not existing:
        ...
        electronics_cat = Category(
            name="Electronics", description="Electronic parts"
        )
        ...
        for product_data in test_products:
            try:
                ...
                logger.warning(
                    f"Product may already exist: {product_data['part_no']}"
                )
...
def test_permissions():
    ...
    has_create_return = perm_service.has_permission(
        cashier.id, "CREATE_RETURN"
    )
    # Cashier might have it via role, but let's check specific permission
    logger.info(
        f"✓ Cashier CREATE_RETURN permission check: {has_create_return}"
    )
...
def test_sales():
    ...
    logger.info(
        f"✓ Sale completed: ID {sale_id}, Invoice: {sale.invoice_number}"
    )

    # Verify inventory was updated
    updated_product = product_service.get_product_by_id(test_product.id)
    assert (
        updated_product.quantity_on_hand == original_qty - 2
    ), "Inventory should be decremented"
    logger.info(
        f"✓ Inventory updated: {original_qty} -> {updated_product.quantity_on_hand}"
    )
...
def test_returns():
    ...
    # Approve return
    assert perm_service.has_permission(
        admin.id, "APPROVE_RETURN"
    ), "Admin should have APPROVE_RETURN"
    ...
    # Process refund
    assert perm_service.has_permission(
        admin.id, "PROCESS_REFUND"
    ), "Admin should have PROCESS_REFUND"
...
def test_complete_workflow():
    ...
    for row in list(pending_before):
        sync_service.mark_synced(
            row["id"], f"SERVER-{row['entity_type']}-{row['entity_id']}"
        )
