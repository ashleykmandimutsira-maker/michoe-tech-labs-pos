...
def icon(name, color=None):
    ...
    data = path.read_bytes().replace(
        b"currentColor", (color or COLORS["muted"]).encode()
    )
...
def card(title, value, caption, accent=None, icon_name=None):
    ...
    if icon_name:
        i = QLabel()
        i.setPixmap(icon(icon_name).pixmap(18, 18))
        top.addWidget(i)
    l.addLayout(top)
    n = QLabel(value)
    n.setObjectName("metricValue")
    l.addWidget(n)
    c = QLabel(caption)
    c.setObjectName("metricCaption")
    l.addWidget(c)
    return f
    ...
    def run(self):
        try:
            ...
            if self.operation == "export":
                result = service.export_inventory(
                    self.path, self.include_cost_price
                )
            elif self.operation == "import":
                result = service.import_inventory(self.path, self.user_id)
            else:
                result = service.generate_template(self.path)
...
class BulkArchiveWorker(QObject):
    ...
    def run(self):
        try:
            ...
            self.progress.emit("Archiving...")
            count = ProductService().archive_all_active_products(
...
class CatalogClearWorker(QObject):
    ...
    def run(self):
        try:
            self.progress.emit("Preparing...")
            self.progress.emit("Clearing catalog...")
            result = ProductService().clear_active_catalog(self.user_id)
            self.progress.emit("Completed")
            self.finished.emit(result)
        except Exception as exc:
            logger.exception("Catalog clear failed")
            self.failed.emit(str(exc))
...
class LoginDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.user = None
        self.setWindowTitle("Sign in - Online Motor Spares POS")
        self.setFixedSize(440, 500)
        self.setStyleSheet(STYLESHEET + "QDialog{background:#FFFFFF;}")
        l = QVBoxLayout(self)
        l.setContentsMargins(42, 36, 42, 30)
        l.setSpacing(12)
        ...
        mark_path = str(
            ...
            / "assets"
            / "logo"
            / "michoe-robot-mark.png"
        )
        mark_pix = QPixmap(mark_path)
        if not mark_pix.isNull():
            mark.setPixmap(
                mark_pix.scaled(
        ...
        b = QLabel("MICHOE TECH LABS")
        b.setStyleSheet(
            f"color:{COLORS['primary']};font-size:15px;font-weight:700;letter-spacing:1px;"
        )
        brand_row.addWidget(b)
        brand_row.addStretch()
        l.addLayout(brand_row)
        h = QLabel("Online Motor Spares POS")
        h.setStyleSheet("font-size:28px;font-weight:700;")
        l.addWidget(h)
        w = QLabel("Welcome back. Sign in to continue.")
        ...
        self.password_eye.setText("Show")
        self.password_eye.setCheckable(True)
        self.password_eye.setToolTip("Show password")
        self.password_eye.clicked.connect(self.toggle_password_visibility)
        password_row.addWidget(self.password_eye)
        l.addLayout(password_row)
        self.error = QLabel("")
        self.error.setStyleSheet(f"color:{COLORS['danger']};")
        l.addWidget(self.error)
        s = QPushButton("SIGN IN")
        s.setObjectName("primary")
        s.setMinimumHeight(44)
        s.clicked.connect(self.login)
        l.addWidget(s)
        self.password.returnPressed.connect(self.login)
        l.addStretch()
        foot = QLabel("Offline-first POS system - Powered by Michoe Tech Labs")
...
class AuthorizationDialog(QDialog):
    ...
    def __init__(
        ...
        action,
        requesting_user_id,
        permission_code="DELETE_PRODUCT",
        parent=None,
    ):
        super().__init__(parent)
        self.auth = AuthenticationService()
        ...
        t = QLabel("Admin authorization")
        t.setObjectName("pageTitle")
        l.addWidget(t)
        d = QLabel(f"{action} requires administrator approval.")
        d.setObjectName("muted")
        l.addWidget(d)
        self.admin = QLineEdit()
        self.admin.setPlaceholderText("Admin username")
        l.addWidget(self.admin)
        self.password = QLineEdit()
        ...
        b.accepted.connect(self.verify)
        b.rejected.connect(self.reject)
        l.addWidget(b)

    def verify(self):
        user = self.auth.authenticate(
            self.admin.text().strip(), self.password.text()
        )
        if (
            not user
            or not user.is_admin()
            or not user.has_permission(self.permission_code)
        ):
            self.error.setText("Valid administrator credentials are required.")
            return
        self.approver = user
        self.accept()
...
class DashboardPage(QWidget):
    def __init__(self, products, user):
        ...
        scroll.setStyleSheet(f"background:{COLORS['canvas']};border:0;")
        ...
        s = QLabel("A live view of sales, stock and operational health.")
        s.setObjectName("muted")
        l.addWidget(s)
        self.period = QComboBox()
        self.period.addItem("Today", "today")
        self.period.addItem("This week", "week")
        self.period.addItem("This month", "month")
        self.period.addItem("This year", "year")
        self.period.addItem("Custom range", "custom")
        self.period.currentIndexChanged.connect(self.refresh)
        self.custom_start = QDateEdit(QDate.currentDate())
        self.custom_start.setCalendarPopup(True)
        self.custom_start.setDisplayFormat("dd MMM yyyy")
        self.custom_start.dateChanged.connect(self.refresh)
        self.custom_start.setVisible(False)
        self.custom_end = QDateEdit(QDate.currentDate())
        self.custom_end.setCalendarPopup(True)
        self.custom_end.setDisplayFormat("dd MMM yyyy")
        self.custom_end.dateChanged.connect(self.refresh)
        self.custom_end.setVisible(False)
        ph = QHBoxLayout()
        ph.addWidget(QLabel("Reporting period"))
        ph.addWidget(self.period)
        ph.addWidget(self.custom_start)
        ph.addWidget(self.custom_end)
        ph.addStretch()
        l.addLayout(ph)
        self.kpis = QGridLayout()
        self.kpis.setSpacing(10)
        l.addLayout(self.kpis)
        row = QHBoxLayout()
        row.setSpacing(14)
        l.addLayout(row)
        top_panel, top_layout = panel("Top selling parts", "This year")
        self.top_table = QTableWidget(0, 4)
        self.top_table.setHorizontalHeaderLabels(
            ["Part", "Units", "Revenue", "Profit"]
        )
        self.top_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
        setup_table(self.top_table)
        top_layout.addWidget(self.top_table)
        row.addWidget(top_panel, 3)
        inv_panel, inv_layout = panel("Inventory health", "Active catalog")
        self.inv_grid = QGridLayout()
        inv_layout.addLayout(self.inv_grid)
        low = QLabel("Low stock watchlist")
        low.setObjectName("sectionTitle")
        inv_layout.addWidget(low)
        ...
        setup_table(self.low_table)
        self.low_table.setMaximumHeight(210)
        inv_layout.addWidget(self.low_table)
        row.addWidget(inv_panel, 2)
        analytics_row = QHBoxLayout()
        analytics_row.setSpacing(14)
        l.addLayout(analytics_row)
        brand_panel, brand_layout = panel("Sales by brand", "This year")
        self.brand_chart = QChartView()
        self.brand_chart.setMinimumHeight(220)
        brand_layout.addWidget(self.brand_chart)
        analytics_row.addWidget(brand_panel, 1)
        payment_panel, payment_layout = panel("Payment mix", "This year")
        self.payment_chart = QChartView()
        self.payment_chart.setMinimumHeight(220)
        payment_layout.addWidget(self.payment_chart)
        analytics_row.addWidget(payment_panel, 1)
        vehicle_panel, vehicle_layout = panel(
            "Sales by vehicle", "Select a make and model"
        )
        self.vehicle_controls = QHBoxLayout()
        self.vehicle_make = QComboBox()
        self.vehicle_model = QComboBox()
        self.vehicle_make.currentIndexChanged.connect(
            self.refresh_vehicle_models
        )
        self.vehicle_model.currentIndexChanged.connect(self.refresh_vehicles)
        self.vehicle_controls.addWidget(self.vehicle_make)
        self.vehicle_controls.addWidget(self.vehicle_model)
        vehicle_layout.addLayout(self.vehicle_controls)
        self.vehicle_table = QTableWidget(0, 3)
        self.vehicle_table.setHorizontalHeaderLabels(
            ["Vehicle", "Units", "Revenue"]
        )
        self.vehicle_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
        setup_table(self.vehicle_table)
        vehicle_layout.addWidget(self.vehicle_table)
        analytics_row.addWidget(vehicle_panel, 1)
        recent_panel, recent_layout = panel(
            "Recent transactions", "Latest completed sales"
        )
        self.recent_table = QTableWidget(0, 5)
        self.recent_table.setHorizontalHeaderLabels(
            ["Invoice", "Customer", "Cashier", "Amount", "Status"]
        )
        self.recent_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
        setup_table(self.recent_table)
        recent_layout.addWidget(self.recent_table)
        l.addWidget(recent_panel)
        sync_panel, sync_layout = panel(
            "Synchronization status", "Offline-first operations"
        )
        sync_row = QHBoxLayout()
        self.sync_status = QLabel("ONLINE - LOCAL DATABASE")
        self.sync_status.setObjectName("statusOnline")
        sync_row.addWidget(self.sync_status)
        sync_row.addStretch()
        self.sync_last = QLabel()
        self.sync_last.setObjectName("muted")
        sync_row.addWidget(self.sync_last)
        sync_layout.addLayout(sync_row)
        self.sync_detail = QLabel()
        self.sync_detail.setObjectName("muted")
        sync_layout.addWidget(self.sync_detail)
        clock_row = QHBoxLayout()
        clock_label = QLabel("System clock (this PC)")
        clock_label.setObjectName("muted")
        clock_row.addWidget(clock_label)
        self.system_clock_value = QLabel()
        self.system_clock_value.setObjectName("metricCaption")
        clock_row.addWidget(self.system_clock_value)
        clock_row.addStretch()
        self.dashboard_refreshed = QLabel()
        self.dashboard_refreshed.setObjectName("muted")
        clock_row.addWidget(self.dashboard_refreshed)
        sync_layout.addLayout(clock_row)
    ...
    def period_args(self):
        p = self.period.currentData() or "today"
        custom = (
            (
    ...
    def refresh(self):
        p, start, end = self.period_args()
        self.custom_start.setVisible(p == "custom")
        self.custom_end.setVisible(p == "custom")
        summary = self.analytics.get_sales_summary(p, start, end)
        inv = self.analytics.get_inventory_summary()
        self.dashboard_refreshed.setText(
            f"Dashboard updated: {system_clock.display_str()}"
        )
        ...
        values = [
            ...
            (
                "Units sold",
                str(summary["units_sold"]),
                "Items leaving stock",
                "package",
            ),
            (
                "Average sale",
                money(summary["average_transaction"]),
                "Average transaction value",
                "invoice",
            ),
            ("Low stock", str(inv["low_stock"]), "Needs attention", "warning"),
            (
                "Out of stock",
                str(inv["out_of_stock"]),
                "Immediate reorder",
                "error",
            ),
            ("Returns", str(summary["returns"]), "Period total", "return"),
            (
                "Inventory value",
                money(inv["stock_selling_value"]),
                "Selling value",
                "package",
            ),
        ]
        for i, (a, b, c, ic) in enumerate(values):
            self.kpis.addWidget(
                card(
                    ...
                    icon_name=(
                        ic
                        if (ROOT / "assets" / "icons" / f"{ic}.svg").exists()
                        else "chart"
                    ),
                ),
                i // 4,
        ...
        while self.inv_grid.count():
            item = self.inv_grid.takeAt(0)
            item.widget().deleteLater() if item.widget() else None
        for i, (label, value) in enumerate(
            [
                ("Products", inv["total_products"]),
                ("Units in stock", inv["units_in_stock"]),
                ("Cost value", money(inv["stock_cost_value"])),
                ("Potential profit", money(inv["potential_profit"])),
            ]
        ):
            self.inv_grid.addWidget(QLabel(label), i, 0)
            v = QLabel(str(value))
            v.setObjectName("metricValue")
            self.inv_grid.addWidget(v, i, 1)
        sync_status = self.sync.get_sync_status()
        last = self.sync.db.execute_query(
            "SELECT MAX(synced_at) last_sync FROM sync_queue WHERE status='SYNCED'"
        )[0]["last_sync"]
        self.sync_last.setText(f"Last sync: {last or 'Not yet'}")
        self.sync_detail.setText(
            f"Pending uploads: {sync_status['PENDING']}   |   Synced: {sync_status['SYNCED']}   |   Failed: {sync_status['FAILED']}"
        )
        self.top_table.setRowCount(0)
        for r, x in enumerate(
            self.analytics.get_top_products(p, 10, start, end)
        ):
            self.top_table.insertRow(r)
            [
                self.top_table.setItem(r, c, QTableWidgetItem(str(v)))
                for c, v in enumerate(
                    [
                        f"{x['part_no']} - {x['description']}",
                        str(x["quantity_sold"]),
                        money(x["revenue"]),
                        money(x["profit"]),
                    ]
        ...
        low = self.analytics.get_low_stock_products()
        self.low_table.setRowCount(len(low))
        for r, x in enumerate(low):
            for c, v in enumerate(
                [x["description"], str(x["current_stock"]), x["status"]]
            ):
                self.low_table.setItem(r, c, QTableWidgetItem(v))
        all_vehicles = self.analytics.get_vehicle_sales(
            p, start=start, end=end
        )
        self.vehicle_make.blockSignals(True)
        current_make = self.vehicle_make.currentData()
        self.vehicle_make.clear()
        self.vehicle_make.addItem("All makes", "")
        for make in sorted({x["make"] for x in all_vehicles}):
            self.vehicle_make.addItem(make, make)
        if current_make is not None:
            idx = self.vehicle_make.findData(current_make)
            self.vehicle_make.setCurrentIndex(idx if idx >= 0 else 0)
    ...
    def refresh_breakdowns(self, p, start=None, end=None):
        brands = self.analytics.get_brand_sales(p, start=start, end=end)
        bchart = QChart()
        bchart.setBackgroundVisible(False)
        bchart.legend().hide()
        bs = QBarSeries()
        bs.setLabelsVisible(True)
        bar = QBarSet("Revenue")
        bar.setColor(QColor(COLORS["primary"]))
        [bar.append(float(x["revenue"])) for x in brands]
        bs.append(bar)
        bchart.addSeries(bs)
        bx = QBarCategoryAxis()
        bx.append([x["brand"] for x in brands])
        bchart.addAxis(bx, Qt.AlignBottom)
        by = QValueAxis()
        by.setRange(0, max([float(x["revenue"]) for x in brands] or [1]) * 1.2)
        bchart.addAxis(by, Qt.AlignLeft)
        bs.attachAxis(bx)
        bs.attachAxis(by)
        self.brand_chart.setChart(bchart)
        payments = self.analytics.get_payment_method_sales(p, start, end)
        pchart = QChart()
        pchart.setBackgroundVisible(False)
        ps = QPieSeries()
        [
            ps.append(x["method"].replace("_", " "), float(x["amount"]))
            for x in payments
        ]
        pchart.addSeries(ps)
        pchart.legend().setVisible(bool(payments))
        self.payment_chart.setChart(pchart)

    def refresh_vehicle_models(self):
        p, start, end = self.period_args()
        make = self.vehicle_make.currentData() or ""
        all_vehicles = self.analytics.get_vehicle_sales(
            p, make=make, start=start, end=end
        )
        current = self.vehicle_model.currentData()
        self.vehicle_model.blockSignals(True)
        self.vehicle_model.clear()
        self.vehicle_model.addItem("All models", "")
        for model in sorted({x["model"] for x in all_vehicles}):
            self.vehicle_model.addItem(model, model)
        idx = self.vehicle_model.findData(current)
        self.vehicle_model.setCurrentIndex(idx if idx >= 0 else 0)
    ...
    def refresh_vehicles(self):
        p, start, end = self.period_args()
        make = self.vehicle_make.currentData() or ""
        model = self.vehicle_model.currentData() or ""
        vehicles = self.analytics.get_vehicle_sales(
            p, make=make, model=model, start=start, end=end
        )[:8]
        ...
        for r, x in enumerate(vehicles):
            for c, v in enumerate(
                ...
                self.vehicle_table.setItem(r, c, QTableWidgetItem(v))
...
class CustomerDialog(QDialog):
    ...
    def __init__(self, sales, parent=None, current=None):
        ...
        l.addWidget(title)
        sub = QLabel(
            "These details will appear on the invoice and remain linked to the sale."
        )
        ...
        self.name.setPlaceholderText(
            "Customer name (required for a named invoice)"
        )
    ...
    def accept(self):
        if not self.name.text().strip():
            self.error.setText(
                "Customer name is required for a named invoice."
            )
            return
        super().accept()
...
class PaymentDialog(QDialog):
    ...
    def __init__(self, total, parent=None):
        ...
        self.tendered.setPlaceholderText(
            f"Enter amount received (minimum {money(self.total, self.currency)})"
        )
        ...
        self.payments_table.setMaximumHeight(130)
        l.addWidget(self.payments_table)
        add_payment = QPushButton("Add payment portion")
        add_payment.setIcon(icon("plus"))
        add_payment.clicked.connect(self.add_payment_portion)
        l.addWidget(add_payment)
        self.error = QLabel("")
        self.error.setStyleSheet(f"color:{COLORS['danger']};")
        l.addWidget(self.error)
    ...
    def update_change(self):
        try:
            ...
            self.change.setText(
                money(max(0.0, tendered - self.total), self.currency)
            )
        except ValueError:
    ...
    def update_currency(self):
        ...
        self.currency = {"CASH_USD": "USD", "CASH_ZIG": "ZiG"}.get(
            method, configured_currency()
        )
        self.amount_label.setText(
            f"Total due: {money(self.total, self.currency)}"
        )
        self.tendered.setPlaceholderText(
            f"Enter amount received (minimum {money(self.total, self.currency)})"
        )
    ...
    def accept(self):
        ...
        if paid < self.total - 0.005:
            self.error.setText(
                f"Payment is short by {money(self.total - paid, self.currency)}."
            )
            return
        super().accept()
    ...
    def add_payment_portion(self):
        ...
        remaining = max(
            0.0,
            self.total - sum(payment["amount"] for payment in self.payments),
        )
        amount = (
            self.total
            if method == "STORE_CREDIT"
            else min(tendered, remaining)
        )
        if amount <= 0:
            self.error.setText("No balance remains to add.")
            return
        self.payments.append(
            {
                ...
                "tendered": tendered if method.startswith("CASH") else amount,
            }
        )
        ...
        for column, value in enumerate(
            ...
            self.payments_table.setItem(row, column, QTableWidgetItem(value))
        remaining = max(
            0.0,
            self.total - sum(payment["amount"] for payment in self.payments),
        )
        self.tendered.setText(f"{remaining:.2f}")
        self.error.setText(
            f"Remaining balance: {money(remaining, self.currency)}"
        )
...
class QuantityDialog(QDialog):
    ...
    def __init__(self, product, available, current=0, mode="add", parent=None):
        ...
        l.addWidget(info)
        stock = QLabel(
            f"Available stock: {self.available} | Current in cart: {self.current}"
        )
        ...
        self.quantity.setMaximum(
            max(1, self.available if mode == "add" else self.available)
        )
        ...
        buttons.button(QDialogButtonBox.Ok).setText(
            "Add to cart" if mode == "add" else "Update quantity"
        )
        buttons.accepted.connect(self.accept)
        buttons.rejected.connect(self.reject)
        l.addWidget(buttons)
    ...
    def update_summary(self):
        ...
        self.summary.setText(
            f"Line value: {money(qty * float(self.product.selling_price), self.product.currency)}"
        )
    ...
    def accept(self):
        ...
        if self.mode == "add" and self.current + qty > self.available:
            QMessageBox.warning(
                self,
                "Insufficient stock",
                f"Only {self.available} units are available. You already have {self.current} in the cart.",
            )
            return
        if self.mode == "set" and qty > self.available:
            QMessageBox.warning(
                self,
                "Insufficient stock",
                f"Only {self.available} units are available.",
            )
            return
...
class POSPage(QWidget):
    def __init__(self, products, inventory, user):
        ...
        select_customer_btn.setIcon(icon("search"))
        select_customer_btn.clicked.connect(self.select_customer)
        h.addWidget(select_customer_btn)
        select_vehicle_btn = QPushButton("Select vehicle")
        select_vehicle_btn.setIcon(icon("package"))
        select_vehicle_btn.clicked.connect(self.select_vehicle)
        h.addWidget(select_vehicle_btn)
        l.addLayout(h)

        search_row = QHBoxLayout()
        self.scan = QLineEdit()
        self.scan.setPlaceholderText(
            "Scan barcode, enter Part No. or Product ID"
        )
        ...
        clear_search.clicked.connect(
            lambda: (self.scan.clear(), self.search(""))
        )
        ...
        self.customer_detail = QLabel(
            "Walk-in customer | No vehicle details captured"
        )
        ...
        for key, handler in (
            ("F2", lambda: self.scan.setFocus()),
            ("F4", self.edit_customer),
            ("F6", self.change_selected_quantity),
            ("F8", self.payment),
            ("F9", self.create_quote),
            ("ESC", self.clear_cart),
        ...
        self.results.setHorizontalHeaderLabels(
            ["Part No", "Description", "Brand", "Vehicle", "Price", "Stock"]
        )
        ...
        self.cart_table.itemActivated.connect(
            lambda _item: self.change_selected_quantity()
        )
        ...
        hold = QPushButton("PARK SALE")
        hold.setIcon(icon("save"))
        hold.clicked.connect(self.hold_sale)
        held_row.addWidget(hold)
        resume = QPushButton("RESUME HELD")
        resume.setIcon(icon("refresh"))
        resume.clicked.connect(self.resume_held_sale)
        held_row.addWidget(resume)
        rl.addLayout(held_row)
        quote = QPushButton("CREATE QUOTATION")
        quote.setIcon(icon("invoice"))
        quote.clicked.connect(self.create_quote)
        rl.addWidget(quote)
        quote_history = QPushButton("QUOTATION HISTORY")
        quote_history.setIcon(icon("invoice"))
        quote_history.clicked.connect(self.quotation_history)
    ...
    def _load_recent_identifiers(self):
        try:
            row = self.sales.db.execute_query(
                "SELECT value FROM settings WHERE key='pos_recent_identifiers' LIMIT 1"
            )
    ...
    def _remember_identifier(self, value):
        ...
        if not value:
            return
        self.recent_identifiers = [value] + [
            x for x in self.recent_identifiers if x != value
        ]
        ...
        try:
            ...
            self.sales.db.execute_update(
                ...
                (
                    "pos_recent_identifiers",
                    json.dumps(self.recent_identifiers),
                    "json",
                ),
            )
        except Exception:
    ...
    def search(self, text):
        ...
        if text.upper().startswith("ID:") and text[3:].strip().isdigit():
            ...
            p = (
                self.products.get_product_by_barcode(text)
                or self.products.get_product_by_part_no(text)
                if text
                else None
            )
            items = (
                ...
                if p
                    ...
                    self.products.search_products(text)
                    if text
                    else self.products.get_all_products()
                )
            )
        ...
        for r, p in enumerate(items):
            ...
            for c, v in enumerate(
                [
                    p.description,
                    p.brand or "",
                    f'{p.vehicle_make or ""} {p.vehicle_model or ""}'.strip(),
    ...
    def scan_product(self):
        ...
        p = None
        p = self.products.get_product_by_barcode(
            term
        ) or self.products.get_product_by_part_no(term)
        if (
            not p
            and term.upper().startswith("ID:")
            and term[3:].strip().isdigit()
        ):
            ...
            if p.id in self.cart:
                ...
                if current >= int(p.quantity_on_hand):
                    QMessageBox.warning(
                        self,
                        "Insufficient stock",
                        f"Only {p.quantity_on_hand} units are available for {p.description}.",
                    )
                else:
    ...
    def add_selected(self):
        ...
        if r < 0:
            QMessageBox.information(
                self,
                "Select a product",
                "Select a product from the search results first.",
            )
            return False
    ...
    def add_to_cart(self, product):
        ...
        if available <= 0:
            QMessageBox.warning(
                self, "Out of stock", f"{product.description} is out of stock."
            )
            return False

        current_qty = int(self.cart.get(product.id, {}).get("quantity", 0))
        dialog = QuantityDialog(
            product, available, current=current_qty, mode="add", parent=self
        )
        ...
        if new_qty > available:
            QMessageBox.warning(
                self,
                "Insufficient stock",
                f"Only {available} units are available for {product.description}.",
            )
            return False
    ...
    def change_selected_quantity(self):
        ...
        if row < 0:
            QMessageBox.information(
                self, "Select an item", "Select a cart item first."
            )
            return
    ...
    def remove_selected(self):
        ...
        if row < 0:
            QMessageBox.information(
                self, "Select an item", "Select a cart item to delete."
            )
            return
        item = self.cart_table.item(row, 0)
        product_id = item.data(Qt.UserRole) if item else None
    ...
    def edit_customer(self):
        ...
        self.customer_detail.setText(
            f'{self.customer["name"]} | {phone_text} | {vehicle_text}'
            + (
                f' | Reg: {self.customer["vehicle_registration"]}'
                if self.customer.get("vehicle_registration")
                else ""
            )
        )
    ...
    def select_customer(self):
        term, accepted = QInputDialog.getText(
            self, "Find customer", "Name, phone, email, code, or company:"
        )
        if not accepted:
            return
        ...
        if not customers:
            QMessageBox.information(
                self,
                "No customers",
                "No active customers matched that search.",
            )
            return
        labels = [
            f'{customer.customer_code} | {customer.name} | {customer.company or customer.phone or ""}'
            for customer in customers
        ]
        selected, accepted = QInputDialog.getItem(
            self, "Select customer", "Customer", labels, 0, False
        )
        if not accepted:
            return
        customer = customers[labels.index(selected)]
        self.customer = customer.to_dict()
        self.selected_vehicle = None
        self.customer_label.setText(
            f"{customer.customer_code} | {customer.name}"
        )
        self.customer_detail.setText(
            f'{customer.name} | {customer.phone or "No phone"} | {customer.company or ""}'
        )
    ...
    def select_vehicle(self):
        if not self.customer or not self.customer.get("id"):
            QMessageBox.information(
                self,
                "Select customer",
                "Select an existing customer before selecting a vehicle.",
            )
            return
        ...
        if not vehicles:
            QMessageBox.information(
                self,
                "No vehicles",
                "This customer has no active vehicles. Create one from Customers.",
            )
            return
        labels = [
            f'{vehicle.registration_number} | {vehicle.make or ""} {vehicle.model or ""}'.strip()
            for vehicle in vehicles
        ]
        selected, accepted = QInputDialog.getItem(
            self, "Select vehicle", "Vehicle", labels, 0, False
        )
        if not accepted:
            return
        vehicle = vehicles[labels.index(selected)]
        self.selected_vehicle = vehicle.to_dict()
        self.customer["vehicle_registration"] = vehicle.registration_number
        self.customer["vehicle_make"] = vehicle.make
        self.customer["vehicle_model"] = vehicle.model
        self.customer_detail.setText(
            f"{self.customer['name']} | {vehicle.registration_number} | {(vehicle.make or '')} {(vehicle.model or '')}".strip()
        )

    def payment(self):
        if not self.cart:
            QMessageBox.information(
                self, "Cart empty", "Add a product before taking payment."
            )
            return
        if self.user and not self.user.has_permission("MAKE_SALE"):
            QMessageBox.warning(
                self,
                "Permission denied",
                "Your role is not allowed to make sales.",
            )
            return
        ...
        cart_total = sum(
            ...
            * (
                1
                + float(
                    self.products.get_product_by_id(product_id).vat_rate or 0
                )
                / 100
            )
        ...
        try:
            ...
            customer_id = customer.get(
                "id"
            ) or self.sales.find_or_create_customer(
                customer["name"],
                customer.get("phone"),
                customer.get("email"),
            ...
            sale = self.sales.create_sale(
                ...
                cashier_name=(
                    self.user.full_name if self.user else "Administrator"
                ),
                vehicle_id=(
                    self.selected_vehicle.get("id")
                    if self.selected_vehicle
                    else None
                ),
            )
            for product_id, item in self.cart.items():
                self.sales.add_item_to_sale(
                    sale, product_id, item["quantity"], item["unit_price"]
                )
            ...
            self.invoice_status.setText(
                f"Invoice: {sale.invoice_number} saved"
            )
            self.last_sale = sale
    ...
    def hold_sale(self):
        if not self.cart:
            QMessageBox.information(
                self, "Cart empty", "Add a product before parking a sale."
            )
            return
        try:
            held = self.sales.hold_sale(
                self.cart, self.customer, self.user.id if self.user else None
            )
            self.cart.clear()
            self.customer = None
            self.refresh_cart()
            ...
            QMessageBox.information(
                self,
                "Sale parked",
                f"{held['hold_number']} is ready to resume.",
            )
        except Exception as exc:
            QMessageBox.critical(self, "Could not park sale", str(exc))

    def resume_held_sale(self):
        held_sales = self.sales.list_held_sales()
        if not held_sales:
            QMessageBox.information(
                self, "No held sales", "There are no parked sales to resume."
            )
            return
        labels = [
            f"{held['hold_number']} | {held['created_at']}"
            for held in held_sales
        ]
        label, accepted = QInputDialog.getItem(
            self, "Resume held sale", "Held sale", labels, 0, False
        )
        if not accepted:
            return
        held = self.sales.release_held_sale(
            held_sales[labels.index(label)]["id"]
        )
        self.cart = held["cart"]
        self.customer = held["customer"] or None
        self.refresh_cart()
        if self.customer:
            self.customer_label.setText(
                self.customer.get("name") or "Walk-in customer"
            )
            self.customer_detail.setText(
                f"{self.customer.get('name') or 'Walk-in customer'} | {self.customer.get('phone') or 'No phone'}"
            )
        QMessageBox.information(
            self,
            "Sale resumed",
            f"{held['hold_number']} has been restored to the cart.",
        )
    ...
    def create_quote(self):
        if not self.cart:
            QMessageBox.information(
                self,
                "Cart empty",
                "Add a product before creating a quotation.",
            )
            return
        customer = self.customer or {}
        try:
            quote = self.sales.create_quotation(
                self.cart,
                customer,
            ...
            quote = self.sales.change_quotation_status(
                quote["id"], "ISSUED", self.user.id if self.user else None
            )
        except Exception as exc:
            QMessageBox.critical(self, "Quotation failed", str(exc))
            return
        ...
        for item in self.cart.values():
            ...
            subtotal += line_total
            rows.append(
                f"<tr><td>{item['part_no']}</td><td>{item['description']}</td><td>{item['quantity']}</td><td>{money(item['unit_price'])}</td><td>{money(line_total)}</td></tr>"
            )
        ...
        print_button = preview.addButton(
            "Print Quotation", QMessageBox.AcceptRole
        )
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save quotation", f"{quote_number}.pdf", "PDF files (*.pdf)"
        )
        if not path:
            return
        printer = QPrinter(QPrinter.HighResolution)
    ...
    def quotation_history(self):
        ...
        if not quotes:
            QMessageBox.information(
                self, "No quotations", "No quotation history is available."
            )
            return
        labels = [
            f"{quote['quote_number']} | {quote['status']} | {money(quote['total'])}"
            for quote in quotes
        ]
        label, accepted = QInputDialog.getItem(
            self, "Quotation history", "Quotation", labels, 0, False
        )
        if not accepted:
            return
        ...
        if quote["status"] in ("DRAFT", "ISSUED"):
            ...
            if action == "Load for payment":
                ...
                self.customer = quote["customer"] or None
                self._loaded_quote_id = quote["id"]
                self.refresh_cart()
                QMessageBox.information(
                    self,
                    "Quotation loaded",
                    "The quotation is in the cart. Take payment to complete the sale.",
                )
                return
        rows = "".join(
            f"<tr><td>{item['part_no']}</td><td>{item['description']}</td><td>{item['quantity']}</td><td>{money(item['line_total'])}</td></tr>"
            for item in quote["items"]
        )
        document = QTextDocument()
        document.setHtml(
            f"<h1>Quotation</h1><p><b>{quote['quote_number']}</b></p><p>{quote['customer'].get('name') or 'Walk-in customer'}</p><table border='1'><tr><th>Part</th><th>Description</th><th>Qty</th><th>Total</th></tr>{rows}</table><p><b>Total: {money(quote['total'])}</b></p>"
        )
        printer = QPrinter(QPrinter.HighResolution)
        dialog = QPrintDialog(printer, self)
        if dialog.exec() == QDialog.Accepted:
...
class InventoryPage(QWidget):
    def __init__(self, products, user=None):
        ...
        self.filter.setPlaceholderText("Search products")
        self.filter.setMaximumWidth(280)
        self.filter.textChanged.connect(self.load)
        h.addWidget(self.filter)
        a = QPushButton("Add product")
        a.setObjectName("primary")
        a.setIcon(icon("plus"))
        a.clicked.connect(self.add_product)
        h.addWidget(a)
        l.addLayout(h)
        can_edit_stock = bool(
            not user
            or user.is_admin()
            or user.has_permission("ADD_STOCK")
            or user.has_permission("ADJUST_STOCK")
        )
        self.delete_btn = QPushButton("Delete selected")
        self.delete_btn.setIcon(icon("delete"))
        self.delete_btn.setEnabled(False)
        self.delete_btn.setToolTip(
            "Permanently delete a product with no sales, stock, or return history"
        )
        self.delete_btn.clicked.connect(self.delete_selected)
        h.addWidget(self.delete_btn)
        sh = QHBoxLayout()
        sh.addWidget(QLabel("Inventory spreadsheet:"))
        sh.addSpacing(4)
        tmpl_btn = QPushButton("Download template")
        tmpl_btn.setIcon(icon("invoice"))
        tmpl_btn.clicked.connect(self.download_template)
        sh.addWidget(tmpl_btn)
        self.import_btn = QPushButton("Import spreadsheet")
        self.import_btn.setIcon(icon("sync"))
        self.import_btn.setEnabled(can_edit_stock)
        self.import_btn.setToolTip(
            "Enter stock once in Excel and bring it into the system"
            if can_edit_stock
            else "Your role cannot import stock changes"
        )
        self.import_btn.clicked.connect(self.import_spreadsheet)
        sh.addWidget(self.import_btn)
        export_btn = QPushButton("Export full inventory")
        export_btn.setIcon(icon("package"))
        export_btn.clicked.connect(self.export_spreadsheet)
        sh.addWidget(export_btn)
        self.undo_import_btn = QPushButton("Undo last inventory import")
        self.undo_import_btn.setIcon(icon("refresh"))
        self.undo_import_btn.setEnabled(bool(user and user.is_admin()))
        self.undo_import_btn.clicked.connect(self.undo_last_import)
        sh.addWidget(self.undo_import_btn)
        sh.addStretch()
        l.addLayout(sh)
        s = products.get_inventory_summary()
        self.summary_grid = QGridLayout()
        self.summary_grid.setSpacing(12)
        l.addLayout(self.summary_grid)
        self._fill_summary(s)
        ...
        self.table.itemActivated.connect(lambda _item: self.delete_selected())
    ...
    def load(self, text=None):
        ...
        for r, p in enumerate(items):
            status = (
                ...
                else "LOW STOCK" if p.is_low_stock() else "IN STOCK"
            )
            vals = [
                p.part_no,
                p.description,
    ...
    def delete_selected(self):
        ...
        answer = QMessageBox.warning(
            ...
            f"Permanently delete {product.part_no}? This cannot be undone. Products with history cannot be permanently deleted.",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No,
        )
        if answer != QMessageBox.Yes:
        ...
        try:
            self.products.delete_product(
                product.id, self.user.id if self.user else 0
            )
    ...
    def add_product(self):
        ...
        if d.exec() != QDialog.Accepted:
            return
        try:
            self.products.create_product(d.value())
            self.load(self.filter.text())
            QMessageBox.information(
                self, "Saved", "Product saved successfully."
            )
        except Exception as e:
            QMessageBox.critical(self, "Could not save product", str(e))
    ...
    def export_spreadsheet(self):
        can_see_cost = bool(
            ...
            or self.user.has_permission("VIEW_COST_PRICE")
        )
        path, _ = QFileDialog.getSaveFileName(
            self,
    ...
    def _run_spreadsheet(
        self, path, operation, button, include_cost_price=True
    ):
        if (
            self._spreadsheet_thread is not None
            and self._spreadsheet_thread.isRunning()
        ):
            return
        self._spreadsheet_thread = QThread(self)
        self._spreadsheet_worker = SpreadsheetWorker(
            path,
            operation,
            include_cost_price,
            self.user.id if self.user else None,
        ...
        self._spreadsheet_worker.finished.connect(
            self._spreadsheet_thread.quit
        )
        ...
        button.setText(
            "Importing inventory..."
            if operation == "import"
            else "Generating inventory spreadsheet..."
        )
    ...
    def _spreadsheet_finished(self, result):
        ...
        self.load(self.filter.text())
        logger.info(
            "UI REFRESH COMPLETE elapsed=%.3fs",
        ...
        if operation == "template":
            QMessageBox.information(
                self,
                "Template ready",
                f"Blank inventory template saved to:\n{path}",
            )
            ...
            QMessageBox.information(
                self, "Exported", f"{result} product(s) exported to:\n{path}"
            )
    ...
    def _spreadsheet_failed(self, error):
        ...
        button.setText(
            "Import spreadsheet"
            if self._spreadsheet_operation == "import"
            else (
                "Download template"
                if self._spreadsheet_operation == "template"
                else "Export full inventory"
            )
        )
    ...
    def import_spreadsheet(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Import inventory spreadsheet", "", "Excel files (*.xlsx)"
        )
        if not path:
            return
    ...
    def undo_last_import(self):
        if not self.user or not self.user.is_admin():
            QMessageBox.warning(
                self,
                "Permission denied",
                "Only an administrator can undo an inventory import.",
            )
            return
        ...
        if not imports:
            QMessageBox.information(
                self,
                "No imports",
                "There are no tracked inventory imports to undo.",
            )
            return
        batch = imports[0]
        if batch["status"] == "UNDONE":
            QMessageBox.information(
                self,
                "Already undone",
                "The latest inventory import has already been undone.",
            )
            return
        answer = QMessageBox.warning(
            self,
            "Undo inventory import",
            f"Undo {batch['filename']}?\n\nRows: {batch['total_rows']}\nNew products: {batch['created_count']}\nUpdated products: {batch['updated_count']}\n\nThis reverses only this import. Historical products and transactions will be preserved.",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No,
        )
        if answer != QMessageBox.Yes:
        ...
        try:
            ...
            QMessageBox.critical(self, "Could not undo import", str(exc))
...
class ProductDialog(QDialog):
    def __init__(self, parent=None, product=None):
        ...
        specs = [
            ...
            ("vehicle_year_from", "Vehicle year from"),
            ("vehicle_year_to", "Vehicle year to"),
            ("cost_price", "Cost price"),
            ("selling_price", "Selling price"),
            ("quantity_on_hand", "Initial stock"),
        ...
        if product:
            for key, w in self.fields.items():
                w.setText(str(getattr(product, key, "")))
        l.addLayout(form)
        self.error = QLabel("")
        self.error.setStyleSheet(f'color:{COLORS["danger"]};')
    ...
    def value(self):
        def num(k, cast=float):
            try:
                return cast(self.fields[k].text() or 0)
            except ValueError:
                raise ValueError(
                    f"{k.replace('_',' ').title()} must be a number"
                )

        return Product(
            id=self.product.id if self.product else None,
            barcode=self.fields["barcode"].text().strip() or None,
            part_no=self.fields["part_no"].text().strip(),
            oem_number=self.fields["oem_number"].text().strip() or None,
            description=self.fields["description"].text().strip(),
            brand=self.fields["brand"].text().strip(),
            vehicle_make=self.fields["vehicle_make"].text().strip() or None,
            vehicle_model=self.fields["vehicle_model"].text().strip() or None,
            vehicle_year_from=(
                num("vehicle_year_from", int)
                if self.fields["vehicle_year_from"].text().strip()
                else None
            ),
            vehicle_year_to=(
                num("vehicle_year_to", int)
                if self.fields["vehicle_year_to"].text().strip()
                else None
            ),
            cost_price=num("cost_price"),
            selling_price=num("selling_price"),
            quantity_on_hand=num("quantity_on_hand", int),
            reorder_level=num("reorder_level", int),
        )
    ...
    def accept(self):
        try:
            ...
            if (
                product.vehicle_year_from is not None
                and product.vehicle_year_to is not None
                and product.vehicle_year_from > product.vehicle_year_to
            ):
                raise ValueError(
                    "Vehicle year from must not be later than vehicle year to."
                )
...
class ProductsPage(QWidget):
    def __init__(self, products, user):
        ...
        self.search_box.textChanged.connect(self.load)
        h.addWidget(self.search_box)
        self.show_archived = QCheckBox("Show archived")
        self.show_archived.toggled.connect(
            lambda _checked: self.load(self.search_box.text())
        )
        h.addWidget(self.show_archived)
        add = QPushButton("Add product")
        add.setObjectName("primary")
        add.setIcon(icon("plus"))
        add.setEnabled(bool(not user or user.has_permission("CREATE_PRODUCT")))
        add.clicked.connect(self.add)
        h.addWidget(add)
        self.archive = QPushButton("Archive selected")
        self.archive.setIcon(icon("archive"))
        self.archive.clicked.connect(self.archive_selected)
        h.addWidget(self.archive)
        self.delete_btn = QPushButton("Permanently delete")
        self.delete_btn.setIcon(icon("delete"))
        self.delete_btn.setToolTip(
            "Permanently delete only products without sale, stock, or return history"
        )
        self.delete_btn.clicked.connect(self.delete_selected)
        h.addWidget(self.delete_btn)
        self.archive_all = QPushButton("Clear active catalog")
        self.archive_all.setIcon(icon("archive"))
        self.archive_all.setToolTip(
            "Deletes unreferenced products and archives products with history"
        )
        self.archive_all.clicked.connect(self.archive_all_active)
        h.addWidget(self.archive_all)
        self.restore = QPushButton("Restore selected")
        self.restore.setIcon(icon("refresh"))
        ...
        self.table.cellDoubleClicked.connect(
            lambda row, _: self.save_product(self.items[row])
        )
        l.addWidget(self.table, 1)
        delete_shortcut = QShortcut(QKeySequence("Delete"), self)
        delete_shortcut.setContext(Qt.WidgetWithChildrenShortcut)
    ...
    def load(self, text=None):
        ...
        all_items = (
            self.products.get_all_products(active_only=False)
        ...
        self.restore.setEnabled(
            bool(
                selected
                and not selected.active
                and self.show_archived.isChecked()
            )
        )
    ...
    def _update_action_buttons(self):
        ...
        self.restore.setEnabled(
            bool(p and not p.active and self.show_archived.isChecked())
        )

    def add(self):
        self.save_product(None)
    ...
    def save_product(self, existing):
        ...
        try:
            p = d.value()
            if not p.part_no or not p.description:
                raise ValueError("Part number and description are required")
            if (
                existing
                and self.user
                and not self.user.has_permission("EDIT_PRODUCT")
            ):
                raise PermissionError(
                    "Your role is not allowed to edit products"
                )
            if existing:
                self.products.update_product(p)
            else:
                self.products.create_product(p)
    ...
    def delete_selected(self):
        ...
        try:
            if (
                not self.user
                or not self.user.has_permission(permission)
                or not self.user.is_admin()
            ):
                dlg = AuthorizationDialog(
                    action, self.user.id if self.user else 0, permission, self
                )
                if dlg.exec() != QDialog.Accepted:
                    return
                approver = dlg.approver.id
                reason = ""
            else:
                reason = ""
            answer = QMessageBox.warning(
                ...
                f"Permanently delete {p.part_no}? This cannot be undone.",
                QMessageBox.Yes | QMessageBox.No,
                QMessageBox.No,
            )
            if answer != QMessageBox.Yes:
                return
            self.products.delete_product(
                p.id, self.user.id if self.user else 0, approver, reason
            )
            self.load(self.search_box.text())
        except Exception as e:
            QMessageBox.critical(self, "Permanent deletion failed", str(e))
    ...
    def archive_all_active(self):
        if (
            not self.user
            or not self.user.is_admin()
            or not self.user.has_permission("DELETE_PRODUCT")
        ):
            QMessageBox.warning(
                self,
                "Permission denied",
        ...
        answer = QMessageBox.warning(
            ...
            "Products with no history will be permanently deleted. Products linked to sales, stock, returns, quotations, or imports will be archived to preserve history. The active catalog will be empty. Continue?",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No,
        )
        if answer != QMessageBox.Yes:
        ...
        confirm, ok = QInputDialog.getText(
            self, "Confirm catalog clear", "Type CLEAR CATALOG to continue:"
        )
        if not ok or confirm.strip().upper() != "CLEAR CATALOG":
            return
        if (
            getattr(self, "_bulk_archive_thread", None)
            and self._bulk_archive_thread.isRunning()
        ):
            return
        self._bulk_archive_thread = QThread(self)
        self._bulk_archive_worker = CatalogClearWorker(self.user.id)
        self._bulk_archive_worker.moveToThread(self._bulk_archive_thread)
        self._bulk_archive_thread.started.connect(
            self._bulk_archive_worker.run
        )
        ...
        self._bulk_archive_worker.finished.connect(
            self._bulk_archive_thread.quit
        )
        self._bulk_archive_worker.failed.connect(
            self._bulk_archive_thread.quit
        )
        self._bulk_archive_thread.finished.connect(self._bulk_archive_cleanup)
        self.archive_all.setEnabled(False)
        self.archive_all.setText("Preparing...")
        self._bulk_archive_thread.start()

    def _bulk_archive_progress(self, status):
        self.archive_all.setText(status)

    def _bulk_archive_finished(self, result):
        self.show_archived.setChecked(False)
        self.load(self.search_box.text())
        self.archive_all.setEnabled(True)
        self.archive_all.setText("Clear active catalog")
        QMessageBox.information(
            self,
            "Catalog cleared",
    ...
    def _bulk_archive_failed(self, error):
        self.archive_all.setEnabled(True)
        self.archive_all.setText("Clear active catalog")
        QMessageBox.critical(self, "Catalog clear failed", error)

    def _bulk_archive_cleanup(self):
        self._bulk_archive_worker.deleteLater()
        self._bulk_archive_thread.deleteLater()
        self._bulk_archive_worker = None
        self._bulk_archive_thread = None

    def _change_state(self, restore):
        p = self._selected()
        if not p:
            return
        permission = "RESTORE_PRODUCT" if restore else "DELETE_PRODUCT"
        action = "Restore product" if restore else "Archive product"
        approver = None
        try:
            if (
                not self.user
                ...
                reason = ""
            if restore:
                self.products.restore_product(
                    p.id, self.user.id if self.user else 0, approver, reason
                )
            else:
                self.products.archive_product(
                    p.id, self.user.id if self.user else 0, approver, reason
                )
            ...
            self.load(self.search_box.text())
            QMessageBox.information(
                self, "Completed", f"{action} completed successfully."
            )
        except Exception as e:
...
class InvoicesPage(QWidget):
    def __init__(self, user=None):
        ...
        h.addStretch()
        self.search_box = QLineEdit()
        self.search_box.setPlaceholderText("Search invoice or customer")
        self.search_box.returnPressed.connect(self.load)
        h.addWidget(self.search_box)
        b = QPushButton("Search")
        b.setIcon(icon("search"))
        b.clicked.connect(self.load)
        h.addWidget(b)
        self.void_btn = QPushButton("Void invoice")
        self.void_btn.setIcon(icon("delete"))
        self.void_btn.setEnabled(bool(user and user.is_admin()))
        self.void_btn.clicked.connect(self.void_selected)
        h.addWidget(self.void_btn)
        self.delete_draft_btn = QPushButton("Delete draft")
        self.delete_draft_btn.setIcon(icon("delete"))
        self.delete_draft_btn.setEnabled(
            bool(user and user.has_permission("invoices.view"))
        )
        self.delete_draft_btn.clicked.connect(self.delete_selected_draft)
        h.addWidget(self.delete_draft_btn)
        l.addLayout(h)
        self.table = QTableWidget(0, 5)
        self.table.setHorizontalHeaderLabels(
            ["Invoice", "Customer", "Subtotal", "Total", "Status"]
        )
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        setup_table(self.table)
        self.table.cellDoubleClicked.connect(self.preview)
        l.addWidget(self.table, 1)
        self.load()

    def load(self):
        rows = (
            self.sales.search_sales(self.search_box.text())
            if self.search_box.text().strip()
            else self.sales.get_recent_sales()
        )
        self.rows = rows
    ...
    def void_selected(self):
        ...
        if not self.user or not self.user.is_admin():
            QMessageBox.warning(
                self,
                "Permission denied",
                "Only an administrator can void invoices.",
            )
            return
        answer = QMessageBox.warning(
            self,
            "Void invoice",
            f"Void {sale.invoice_number}? The invoice and payment history will be kept and stock will be restored.",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No,
        )
        if answer != QMessageBox.Yes:
            return
        try:
            self.sales.void_invoice(sale.invoice_number, self.user.id)
            self.load()
            QMessageBox.information(
                self,
                "Invoice voided",
                f"{sale.invoice_number} was voided and retained in history.",
            )
        except Exception as exc:
            QMessageBox.critical(self, "Could not void invoice", str(exc))
    ...
    def delete_selected_draft(self):
        ...
        answer = QMessageBox.warning(
            ...
            f"Delete draft {sale.invoice_number}? Completed invoices cannot be deleted.",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No,
        )
        if answer != QMessageBox.Yes:
            return
        try:
            self.sales.delete_draft_invoice(sale.invoice_number, self.user.id)
            self.load()
            QMessageBox.information(
                self, "Draft deleted", f"{sale.invoice_number} was deleted."
            )
        except Exception as exc:
            QMessageBox.critical(self, "Could not delete invoice", str(exc))
...
class ReturnsPage(QWidget):
    def __init__(self, user):
        ...
        l = QVBoxLayout(self)
        l.setContentsMargins(28, 24, 28, 24)
        t = QLabel("Returns & Refunds")
        t.setObjectName("pageTitle")
        l.addWidget(t)
        h = QHBoxLayout()
        self.invoice = QLineEdit()
        self.invoice.setPlaceholderText("Search original invoice")
        h.addWidget(self.invoice)
        find = QPushButton("Find invoice")
        find.setObjectName("primary")
        find.setIcon(icon("search"))
        find.clicked.connect(self.find)
        h.addWidget(find)
        l.addLayout(h)
        self.info = QLabel("Enter an invoice number to begin a return.")
        self.info.setObjectName("muted")
        l.addWidget(self.info)
        self.item_selector = QComboBox()
        l.addWidget(self.item_selector)
        self.return_quantity = QSpinBox()
        self.return_quantity.setMinimum(1)
        self.return_quantity.setEnabled(False)
        l.addWidget(self.return_quantity)
        self.reason = QLineEdit()
        self.reason.setPlaceholderText("Return reason")
        l.addWidget(self.reason)
        self.process = QPushButton("Create return and refund")
        self.process.setObjectName("primary")
        self.process.setEnabled(False)
        self.process.clicked.connect(self.create)
        l.addWidget(self.process)
        l.addStretch()

    def find(self):
        matches = self.returns.search_original_invoices(
            self.invoice.text().strip()
        )
        if len(matches) > 1:
            labels = [
                f"{row['invoice_number']} | {row['customer_name'] or ''} | {row['vehicle_registration'] or ''} | {money(row['total'])}"
                for row in matches
            ]
            selected, accepted = QInputDialog.getItem(
                self, "Search Original Invoice", "Invoice", labels, 0, False
            )
            if not accepted:
                return
            self.invoice.setText(
                matches[labels.index(selected)]["invoice_number"]
            )
        elif len(matches) == 1:
            self.invoice.setText(matches[0]["invoice_number"])
        self.sale = self.returns.find_original_sale(
            self.invoice.text().strip()
        )
        if not self.sale:
            self.info.setText("Invoice not found.")
            self.process.setEnabled(False)
            return
        self.item_selector.clear()
        for item in self.sale.items:
            self.item_selector.addItem(
                f"{item.part_no} - {item.product_name} (sold {item.quantity})",
                item.id,
            )
        self.item_selector.currentIndexChanged.connect(
            self._update_return_quantity
        )
        self.info.setText(
            f"Invoice {self.sale.invoice_number} - {len(self.sale.items)} item(s) - Total {money(self.sale.total)}"
        )
        self.process.setEnabled(bool(self.sale.items))
        self._update_return_quantity()

    def _update_return_quantity(self):
        if (
            not getattr(self, "sale", None)
            or self.item_selector.currentIndex() < 0
        ):
            return
        item = self.sale.items[self.item_selector.currentIndex()]
        remaining = self.returns.calculate_returnable_quantity(
            item.product_id, item.quantity, self.sale.id
        )
        self.return_quantity.setMaximum(max(1, remaining))
        self.return_quantity.setEnabled(remaining > 0)
        self.process.setEnabled(remaining > 0)

    def create(self):
        try:
            item = self.sale.items[self.item_selector.currentIndex()]
            obj = self.returns.create_return(
                self.sale.invoice_number,
                [
                    {
                        "product_id": item.product_id,
                        "quantity": self.return_quantity.value(),
                        "condition": "GOOD",
                        "return_reason": self.reason.text()
                        or "Customer return",
                    }
                ],
                self.reason.text() or "Customer return",
                self.user.id,
            )
            rid = self.returns.save_return(obj)
            self.returns.approve_return(rid, self.user.id)
            self.returns.process_return(rid, self.user.id)
            self.returns.process_refund(rid, "CASH_USD", self.user.id)
            QMessageBox.information(
                self,
                "Return complete",
                f"Return {obj.return_number} was processed.",
            )
            self.process.setEnabled(False)
        except Exception as e:
...
class SyncPage(QWidget):
    def __init__(self, user=None):
        ...
        sub = QLabel(
            "Sales, stock changes and product updates are saved locally first and synchronized when the server is reachable."
        )
        sub.setObjectName("muted")
        ...
        activity_panel, activity_layout = panel(
            "Synchronization activity", "Latest local queue records"
        )
        self.table = QTableWidget(0, 7)
        self.table.setHorizontalHeaderLabels(
            [
    ...
    def _authorized(self):
        return bool(
            self.user
            and (self.user.is_admin() or self.user.has_permission("SYNC_DATA"))
        )

    def load(self):
        status = self.sync.get_sync_status()
        last_rows = self.sync.db.execute_query(
            "SELECT MAX(synced_at) last_sync FROM sync_queue WHERE status='SYNCED'"
        )
        last = last_rows[0]["last_sync"] if last_rows else None
        state = self.sync.connection_status()
        self.connection.setText(state)
        self.connection.setObjectName(
            "statusOnline"
            if state == "ONLINE"
            else "statusWarning" if state == "OFFLINE" else "muted"
        )
        ...
        rows = self.sync.db.execute_query(
            "SELECT entity_type,entity_id,operation,status,COALESCE(synced_at,last_attempt,created_at) updated_at,external_id,error_message FROM sync_queue ORDER BY created_at DESC LIMIT 50"
        )
        self.table.setRowCount(len(rows))
    ...
    def sync_now(self):
        if not self._authorized():
            QMessageBox.warning(
                self,
                "Permission denied",
                "You do not have permission to synchronize data.",
            )
            return
        ...
        if result["status"] == "OFFLINE":
            QMessageBox.warning(
                self, "Synchronization unavailable", result["message"]
            )
        elif result["status"] == "NOT_CONFIGURED":
            QMessageBox.warning(
                self, "Synchronization not configured", result["message"]
            )
        else:
            QMessageBox.information(
                self,
    ...
    def retry(self):
        if not self._authorized():
            QMessageBox.warning(
                self,
                "Permission denied",
                "You do not have permission to retry synchronization.",
            )
            return
...
class ReportsPage(QWidget):
    def __init__(self, user=None):
        ...
        h = QHBoxLayout()
        self.filter = QLineEdit()
        self.filter.setPlaceholderText("Filter by invoice, customer, vehicle")
        h.addWidget(self.filter)
        b = QPushButton("Run sales report")
        b.setObjectName("primary")
        b.setIcon(icon("chart"))
        b.clicked.connect(self.run)
        h.addWidget(b)
        self.tax_btn = QPushButton("Tax & ZIMRA Reports")
        self.tax_btn.setIcon(icon("invoice"))
        self.tax_btn.setEnabled(bool(user and user.is_admin()))
        self.tax_btn.clicked.connect(self.tax_report)
        h.addWidget(self.tax_btn)
        l.addLayout(h)
        self.tax_period = QComboBox()
        self.tax_period.addItems(
            ["Today", "This Week", "This Month", "This Year", "Custom Range"]
        )
        self.tax_start = QDateEdit(QDate.currentDate())
        self.tax_end = QDateEdit(QDate.currentDate())
        self.tax_start.setCalendarPopup(True)
        self.tax_end.setCalendarPopup(True)
        self.tax_start.setVisible(False)
        self.tax_end.setVisible(False)
        self.tax_period.currentIndexChanged.connect(self._tax_dates_changed)
        dates = QHBoxLayout()
        dates.addWidget(QLabel("Tax period"))
        dates.addWidget(self.tax_period)
        dates.addWidget(self.tax_start)
        dates.addWidget(self.tax_end)
        self.tax_print = QPushButton("Print tax report")
        self.tax_print.setIcon(icon("print"))
        self.tax_print.setEnabled(False)
        self.tax_print.clicked.connect(self.print_tax)
        dates.addWidget(self.tax_print)
        self.tax_pdf = QPushButton("Save PDF")
        self.tax_pdf.setIcon(icon("save"))
        self.tax_pdf.setEnabled(False)
        self.tax_pdf.clicked.connect(self.save_tax_pdf)
        dates.addWidget(self.tax_pdf)
        self.tax_csv = QPushButton("Export CSV")
        self.tax_csv.setIcon(icon("invoice"))
        self.tax_csv.setEnabled(False)
        self.tax_csv.clicked.connect(self.export_tax_csv)
        dates.addWidget(self.tax_csv)
        self.tax_xlsx = QPushButton("Export Excel")
        self.tax_xlsx.setIcon(icon("invoice"))
        self.tax_xlsx.setEnabled(False)
        self.tax_xlsx.clicked.connect(self.export_tax_xlsx)
        dates.addWidget(self.tax_xlsx)
        dates.addStretch()
        l.addLayout(dates)
    ...
    def run(self):
        rows = (
            self.sales.search_sales(self.filter.text())
            if self.filter.text().strip()
            else self.sales.get_recent_sales(100)
        )
        revenue = sum(s.total for s in rows)
        analytics = AnalyticsService()
        daily = analytics.get_daily_sales_report()
        top = analytics.get_top_products("year", 10)
        stock = analytics.get_stock_report()
        customers = analytics.get_customer_sales()
        returns = analytics.get_returns_report()
        lines = [
            f"Sales report\n\nTransactions: {len(rows)}\nRevenue: {money(revenue)}\n\nRecent sales\n"
            + "\n".join(
                f"{s.invoice_number}  {s.customer_name}  {money(s.total)}"
                for s in rows
            ),
            "Daily sales\n"
            + "\n".join(
                f"{x['day']}  sales={x['sales']}  gross={money(x['gross_sales'])}  returns={money(x['returns'])}  net={money(x['net_sales'])}  profit={money(x['profit'])}"
                for x in daily
            )
            or "No sales in the selected period.",
            "Top selling parts\n"
            + "\n".join(
                f"{x['part_no']}  qty={x['quantity_sold']}  revenue={money(x['revenue'])}  profit={money(x['profit'])}"
                for x in top
            )
            or "No product sales.",
            "Stock report\n"
            + "\n".join(
                f"{x['part_no']}  {x['description']}  stock={x['current_stock']}  value={money(x['stock_value'])}  {x['stock_status']}"
                for x in stock
            ),
            "Customer sales\n"
            + "\n".join(
                f"{x['customer']}  purchases={x['purchases']}  spent={money(x['total_spent'])}"
                for x in customers
            ),
            "Returns report\n"
            + "\n".join(
                f"{x['return_number']}  {x['invoice'] or ''}  {x['product']}  qty={x['quantity']}  refund={money(x['refund'])}  {x['reason'] or ''}"
                for x in returns
            ),
        ]
    ...
    def tax_report(self):
        if not self.user or not self.user.is_admin():
            QMessageBox.warning(
                self,
                "Permission denied",
                "Only an administrator can view tax reports.",
            )
            return
        from services.tax_report_service import TaxReportService

        start, end = self._tax_dates()
        report = TaxReportService().summary(start, end)
        sales = report["sales"]
        self.tax_text = (
            f"Tax & ZIMRA Report ({start} to {end}; database summary, not FDMS fiscalised)\n\nInvoices: {sales['invoices']}\nGross sales: {money(sales['gross_sales'])}\nTaxable sales: {money(sales['taxable_sales'])}\nOutput VAT: {money(sales['output_vat'])}\nDiscounts: {money(sales['discounts'])}\nReturns / credit notes: {money(report['returns'])}\nNet sales: {money(report['net_sales'])}\nZero-rated sales: {money(report['zero_rated_sales'])}\nExempt sales: {money(report['exempt_sales'])}\n\nPayment methods\n"
            + "\n".join(
                f"{row['method']}: {money(row['amount'])}"
                for row in report["payments"]
            )
            + "\n\nInvoice register\n"
            + "\n".join(
                f"{row['invoice_number']}  {row['created_at']}  {row['customer_name'] or ''}  VAT {money(row['vat_amount'])}  Total {money(row['total'])}"
                for row in report["invoice_register"]
            )
        )
        self.tax_rows = report["invoice_register"]
        self.output.setPlainText(self.tax_text)
        self.tax_print.setEnabled(True)
        self.tax_pdf.setEnabled(True)
        self.tax_csv.setEnabled(True)
        self.tax_xlsx.setEnabled(True)

    def _tax_dates_changed(self):
        custom = self.tax_period.currentText() == "Custom Range"
        self.tax_start.setVisible(custom)
        self.tax_end.setVisible(custom)

    def _tax_dates(self):
        from datetime import date, timedelta

        today = date.today()
        period = self.tax_period.currentText()
        if period == "Today":
            start = today
            ...
            return self.tax_start.date().toString(
                "yyyy-MM-dd"
            ), self.tax_end.date().toString("yyyy-MM-dd")
        return start.isoformat(), today.isoformat()

    def _tax_document(self):
        document = QTextDocument()
        document.setPlainText(getattr(self, "tax_text", ""))
        return document
    ...
    def print_tax(self):
        ...
        dialog = QPrintDialog(printer, self)
        if dialog.exec() == QDialog.Accepted:
            self._tax_document().print_(printer)

    def save_tax_pdf(self):
        path, _ = QFileDialog.getSaveFileName(
            self,
            "Save tax report",
            "tax_zimra_report.pdf",
            "PDF files (*.pdf)",
        )
        if path:
            printer = QPrinter(QPrinter.HighResolution)
            ...
            AuditService().log_action(
                "TAX_REPORT_PDF_EXPORTED", "REPORT", None, self.user.id
            )
    ...
    def export_tax_csv(self):
        ...
        if path:
            ...
            with open(path, "w", newline="", encoding="utf-8") as output:
                ...
                writer.writeheader()
                writer.writerows(getattr(self, "tax_rows", []))
            AuditService().log_action(
                "TAX_REPORT_CSV_EXPORTED", "REPORT", None, self.user.id
            )
    ...
    def export_tax_xlsx(self):
        ...
        if path:
            ...
            start, end = self._tax_dates()
            TaxReportService().export_xlsx(path, start, end)
            AuditService().log_action(
                "TAX_REPORT_XLSX_EXPORTED", "REPORT", None, self.user.id
            )
...
class UserPrivilegesDialog(QDialog):
    ...
    def __init__(self, admin_user, target_user, parent=None):
        ...
        info = QLabel(
            f"Role: {role}  |  Select the direct privileges this user should have."
        )
        info.setObjectName("muted")
        layout.addWidget(info)
        if role != "ADMIN":
            note = QLabel(
                "Product archive/restore remain ADMIN-only for security."
            )
        ...
        enabled = {
            p.code for p in self.auth.get_user_permissions(target_user.id)
        }
        ...
        for category, items in groups.items():
            frame, frame_layout = panel(
                category.title(), f"{len(items)} permissions"
            )
            grid = QGridLayout()
            ...
            for index, perm in enumerate(items):
                ...
                if role != "ADMIN" and perm.code in (
                    "DELETE_PRODUCT",
                    "RESTORE_PRODUCT",
                ):
    ...
    def save(self):
        selected = [
            code for code, box in self.checks.items() if box.isChecked()
        ]
        try:
...
class UsersPage(QWidget):
    def __init__(self, current_user=None):
        ...
        self.table.setHorizontalHeaderLabels(
            ["Username", "Full name", "Role", "Phone", "Status", "Privileges"]
        )
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        setup_table(self.table)
        self.table.itemSelectionChanged.connect(self._update_actions)
        self.table.cellDoubleClicked.connect(
            lambda row, _col: self._open_privileges(row)
        )
    ...
    def _update_actions(self):
        ...
        self.delete_user_btn.setEnabled(
            bool(
                self.current_user
                and self.current_user.is_admin()
                and self.current_user.has_permission("MANAGE_USERS")
                and selected
                and selected.id != self.current_user.id
            )
        )
    ...
    def add_user(self):
        if (
            ...
            or not self.current_user.has_permission("MANAGE_USERS")
        ):
            QMessageBox.warning(
                self,
                "Permission denied",
                "Only an authorized administrator can create users.",
            )
            return
        ...
        try:
            ...
            QMessageBox.information(
                self,
                "User created",
                f"{data['full_name']} was created successfully.",
            )
        except Exception as exc:
    ...
    def change_role(self):
        ...
        if not user:
            QMessageBox.information(
                self, "Select a user", "Select a user first."
            )
            return
        if not self._can_manage():
            QMessageBox.warning(
                self,
                "Permission denied",
                "Only an authorized administrator can change user roles.",
            )
            return
        if user.id == self.current_user.id:
            QMessageBox.warning(
                self,
                "Protected account",
                "The currently logged-in administrator role cannot be changed here.",
            )
            return
        roles = self.auth.get_all_roles()
        labels = [r.name.replace("_", " ").title() for r in roles]
        current_index = next(
            (i for i, r in enumerate(roles) if r.id == user.role_id), 0
        )
        choice, ok = QInputDialog.getItem(
            self,
            "Change role",
            f"Role for {user.full_name}:",
            labels,
            current_index,
            False,
        )
        ...
        try:
            ...
            QMessageBox.information(
                self,
                "Role updated",
                f'{user.full_name} is now {role.name.replace("_"," ").title()}.',
            )
        except Exception as exc:
    ...
    def delete_user(self):
        ...
        if (
            ...
            QMessageBox.warning(
                self,
                "Permission denied",
                "Only an authorized administrator can delete users.",
            )
            return
        if user.id == self.current_user.id:
            QMessageBox.warning(
                self,
                "Protected account",
                "The currently logged-in administrator cannot be deleted.",
            )
            return
        answer = QMessageBox.warning(
            self,
        ...
        reason, ok = QInputDialog.getText(
            self, "Delete user access", "Reason (required):"
        )
        if not ok or not reason.strip():
            return
        try:
            self.auth.delete_user(
                self.current_user.id, user.id, reason.strip()
            )
            self.load()
            QMessageBox.information(
                self,
                "User deleted",
                f"Access for {user.username} has been disabled.",
            )
        except Exception as exc:
...
class PermissionsPage(QWidget):
    ...
    def __init__(self):
        ...
        layout.addWidget(title)
        desc = QLabel(
            "Permission catalogue. Use Users > Manage privileges to grant access to individual users."
        )
        ...
        for category, permissions in groups.items():
            frame, frame_layout = panel(
                category.title(), f"{len(permissions)} permissions"
            )
            ...
            for row, perm in enumerate(permissions):
                table.setItem(
                    row,
                    0,
                    QTableWidgetItem(perm.code.replace("_", " ").title()),
                )
...
class SettingsPage(QWidget):
    def __init__(self, user=None):
        ...
        l = QVBoxLayout(self)
        l.setContentsMargins(28, 22, 28, 28)
        l.setSpacing(14)
        t = QLabel("Settings")
        t.setObjectName("pageTitle")
        l.addWidget(t)
        d = QLabel(
            "Manage business defaults and offline-first operating preferences."
        )
        d.setObjectName("muted")
        l.addWidget(d)
        grid = QGridLayout()
        grid.setSpacing(12)
        l.addLayout(grid)
        company, company_layout = panel(
            "Company profile", "Shown on receipts and invoices"
        )
        form = QFormLayout()
        self.shop = QLineEdit()
        self.shop.setPlaceholderText("Online Motor Spares")
        form.addRow("Company name", self.shop)
        self.tagline = QLineEdit()
        self.tagline.setPlaceholderText("Business tagline")
        form.addRow("Tagline", self.tagline)
        self.phone = QLineEdit()
        self.phone.setPlaceholderText("Contact phone")
        form.addRow("Contact phone", self.phone)
        company_layout.addLayout(form)
        grid.addWidget(company, 0, 0)
        operations, operations_layout = panel(
            "Sales and currency", "Defaults for new transactions"
        )
        form = QFormLayout()
        self.currency = QComboBox()
        self.currency.addItems(CurrencyService.CODES)
        form.addRow("Default currency", self.currency)
        self.tax = QLineEdit()
        self.tax.setPlaceholderText("15")
        form.addRow("VAT rate (%)", self.tax)
        self.receipt = QCheckBox("Print receipt after completed sale")
        form.addRow("", self.receipt)
        operations_layout.addLayout(form)
        grid.addWidget(operations, 0, 1)
        sync_panel, sync_layout = panel(
            "Offline and synchronization", "Local-first reliability"
        )
        form = QFormLayout()
        self.sync_mode = QComboBox()
        self.sync_mode.addItems(["Offline-first", "Online preferred"])
        form.addRow("Operating mode", self.sync_mode)
        self.endpoint = QLineEdit()
        self.endpoint.setPlaceholderText("Optional sync endpoint")
        form.addRow("Sync endpoint", self.endpoint)
        sync_layout.addLayout(form)
        grid.addWidget(sync_panel, 1, 0)
        printer, printer_layout = panel(
            "Printer and backup", "Hardware and data safety"
        )
        form = QFormLayout()
        self.printer = QLineEdit()
        self.printer.setPlaceholderText("Default printer name")
        form.addRow("Receipt printer", self.printer)
        self.backup = QCheckBox("Remind me to back up the database")
        form.addRow("", self.backup)
        printer_layout.addLayout(form)
        grid.addWidget(printer, 1, 1)
        if user and user.is_admin():
            cleanup, cleanup_layout = panel(
                "System Reset / Data Cleanup",
                "Backup-first administrative cleanup; financial history is voided, not deleted",
            )
            cleanup_btn = QPushButton("System Reset / Data Cleanup")
            cleanup_btn.setIcon(icon("warning"))
    ...
    def load(self):
        rows = self.db.execute_query("SELECT key,value FROM settings")
        values = {x["key"]: x["value"] for x in rows}
        self.shop.setText(values.get("company_name") or "Online Motor Spares")
        self.tagline.setText(values.get("tagline", ""))
        self.phone.setText(values.get("company_phone", ""))
        self.currency.setCurrentText(
            values.get("currency") or configured_currency()
        )
        self.tax.setText(values.get("tax_rate", "15"))
        self.sync_mode.setCurrentText(values.get("sync_mode", "Offline-first"))
        self.endpoint.setText(values.get("sync_endpoint", ""))
        self.printer.setText(values.get("printer_name", ""))
        self.receipt.setChecked(values.get("auto_print", "0") == "1")
        self.backup.setChecked(values.get("backup_reminder", "0") == "1")
    ...
    def save(self):
        try:
            values = [
                ...
                ("auto_print", "1" if self.receipt.isChecked() else "0"),
                ("backup_reminder", "1" if self.backup.isChecked() else "0"),
            ]
    ...
    def run_cleanup(self):
        if not self.user or not self.user.is_admin():
            QMessageBox.warning(
                self,
                "Permission denied",
                "Only an administrator can run system cleanup.",
            )
            return
        options = [
            "Clear inventory (archive active products)",
            "Clear customers (archive customers and vehicles)",
            "Clear quotations (cancel open quotations)",
            "Clear sales/invoices (void completed invoices and restore stock)",
            "Full system reset (all of the above)",
        ]
        choice, ok = QInputDialog.getItem(
            self,
            "System Reset / Data Cleanup",
        ...
        confirm, ok = QInputDialog.getText(
            ...
            "Type RESET SYSTEM to create a backup and continue:",
        )
        ...
        option = {
            ...
            "Clear quotations (cancel open quotations)": "quotations",
            "Clear sales/invoices (void completed invoices and restore stock)": "sales_invoices",
            "Full system reset (all of the above)": "full",
        }[choice]
        try:
...
class CustomersPage(QWidget):
    def __init__(self):
        ...
        h.addStretch()
        self.search_box = QLineEdit()
        self.search_box.setPlaceholderText("Search name, phone, email or city")
        self.search_box.returnPressed.connect(self.load)
        h.addWidget(self.search_box)
        find = QPushButton("Search")
        find.setIcon(icon("search"))
        find.clicked.connect(self.load)
        h.addWidget(find)
    ...
    def load(self):
        term = f"%{self.search_box.text().strip()}%"
        rows = self.db.execute_query(
            "SELECT id,name,phone,email,city FROM customers WHERE name LIKE ? OR phone LIKE ? OR email LIKE ? OR city LIKE ? ORDER BY name",
            (term, term, term, term),
        )
        self.rows = rows
        self.table.setRowCount(len(rows))
    ...
    def add(self):
        ...
        values = d.values()
        try:
            self.sales.find_or_create_customer(
                values["name"],
                values.get("phone"),
                values.get("email"),
            ...
            self.load()
            QMessageBox.information(
                self, "Saved", "Customer saved successfully."
            )
        except Exception as e:
            QMessageBox.critical(self, "Could not save customer", str(e))
    ...
    def edit(self, row, column):
        ...
        try:
            ...
            QMessageBox.critical(self, "Could not update customer", str(e))
...
class PlaceholderPage(QWidget):
    def __init__(self, title, description):
        ...
        pl.addWidget(
            QLabel(
                "This workspace is connected to the existing backend services."
            )
        )
        pl.addStretch()
        l.addWidget(p, 1)
...
class MotorSparesPOSTestWindow(QMainWindow):
    ...
    def __init__(self, user=None):
        super().__init__()
        self.user = user
        self.setWindowTitle("Online Motor Spares POS - Michoe Tech Labs")
        ...
        subscribe_data_changed(self._on_data_changed)

    def build(self):
        c = QWidget()
        c.setObjectName("canvas")
        ...
        side.setStyleSheet(f'background:{COLORS["nav"]};')
        sl = QVBoxLayout(side)
        sl.setContentsMargins(18, 24, 18, 18)
        brand = QLabel("MICHOE TECH LABS")
        brand.setObjectName("brand")
        sl.addWidget(brand)
        sub = QLabel("Online Motor Spares")
        ...
        if self._can_access_dashboard():
            entries.append(
                (
                    "Dashboard",
                    DashboardPage(self.products, self.user),
                    "dashboard",
                )
            )
        entries.extend(
            [
                (
                    "POS",
                    POSPage(self.products, self.inventory, self.user),
                    "cart",
                ),
                (
                    "Inventory",
                    InventoryPage(self.products, self.user),
                    "package",
                ),
                (
                    "Products",
                    ProductsPage(self.products, self.user),
                    "package",
                ),
                ("Invoices", InvoicesPage(self.user), "invoice"),
                ("Returns", ReturnsPage(self.user), "return"),
                ("Customers", CustomersPage(), "users"),
                ("Reports", ReportsPage(self.user), "chart"),
            ]
        )
        for name, page, ic in entries:
            self.add_nav(sl, name, page, ic)
        al = QLabel("ADMINISTRATION")
        al.setStyleSheet(
            f'color:{COLORS["nav_muted"]};font-size:10px;font-weight:700;padding:16px 6px 6px;'
        )
        sl.addWidget(al)
        if self.user and (
            self.user.is_admin() or self.user.has_permission("MANAGE_USERS")
        ):
            self.add_nav(sl, "Users", UsersPage(self.user), "users")
        if self.user and (
            self.user.is_admin()
            or self.user.has_permission("MANAGE_PERMISSIONS")
        ):
            self.add_nav(sl, "Permissions", PermissionsPage(), "shield")
        if self.user and (
            self.user.is_admin() or self.user.has_permission("SYSTEM_SETTINGS")
        ):
            self.add_nav(sl, "Settings", SettingsPage(self.user), "settings")
        if self.user and (
            self.user.is_admin() or self.user.has_permission("SYNC_DATA")
        ):
            self.add_nav(sl, "Synchronization", SyncPage(self.user), "sync")
        sl.addStretch()
        out = QPushButton("Logout")
        out.setObjectName("nav")
        out.setIcon(icon("logout"))
        ...
        tl = QHBoxLayout(top)
        tl.setContentsMargins(28, 0, 24, 0)
        self.header = QLabel(
            "Dashboard" if "Dashboard" in self.pages else "POS"
        )
        self.header.setObjectName("sectionTitle")
        tl.addWidget(self.header)
        tl.addStretch()
        online = QLabel("ONLINE")
        online.setObjectName("statusOnline")
        tl.addWidget(online)
        who = QLabel(
            f'{self.user.full_name if self.user else "Administrator"}  |  {self.user.role.name if self.user and self.user.role else "ADMIN"}'
        )
        ...
        self.go("Dashboard" if "Dashboard" in self.pages else "POS")
    ...
    def _setup_auto_sync(self):
        ...
        try:
            configured = float(
                self._sync_service.config.get("sync_interval", 60)
            )
    ...
    def _auto_sync(self):
        ...
        threading.Thread(
            target=worker, name="POS-AutoSync", daemon=True
        ).start()
    ...
    def add_nav(self, layout, name, page, icon_name=None):
        ...
        b.setObjectName("nav")
        b.setCheckable(True)
        b.setIcon(icon(icon_name) if icon_name else QIcon())
        b.setIconSize(QSize(18, 18))
        b.clicked.connect(lambda _, n=name: self.go(n))
        layout.addWidget(b)
        self.nav[name] = b
    ...
    def _can_access_dashboard(self):
        return bool(
            ...
            and (
                self.user.is_admin()
                or self.user.has_permission("dashboard.view")
            )
        )
    ...
    def go(self, name):
        if name == "Dashboard" and not self._can_access_dashboard():
            ...
            name = (
                "POS" if "POS" in self.pages else next(iter(self.pages), None)
            )
            if name is None:
                return False
