...
class InvoiceLifecycleTests(unittest.TestCase):
    ...
    def test_paid_invoice_is_voided_but_preserved_with_audit(self):
        self.assertTrue(
            self.sales.void_invoice(
                self.sale.invoice_number, 1, "Customer cancellation"
            )
        )
        ...
        self.assertEqual(
            "VOIDED", self.sales.get_sale_by_id(self.sale.id).status
        )
        self.assertEqual(
            5,
            self.products.get_product_by_id(self.product_id).quantity_on_hand,
        )
        self.assertEqual(
            1,
            self.sales.db.execute_query(
                "SELECT COUNT(*) count FROM payments WHERE sale_id=?",
                (self.sale.id,),
            )[0]["count"],
        )
        self.assertTrue(
            self.sales.db.execute_query(
                "SELECT 1 FROM audit_logs WHERE action='INVOICE_VOIDED' AND entity_type='INVOICE'"
            )
        )
    ...
    def test_draft_invoice_can_be_deleted_without_deleting_sale(self):
        ...
        self.sales.db.execute_update(
            "INSERT INTO invoices(invoice_number,sale_id,customer_name,subtotal,vat_amount,total,status) VALUES(?,?,?,?,?,?, 'DRAFT')",
            (draft, self.sale.id, "Invoice Customer", 0, 0, 0),
        )
        self.assertTrue(self.sales.delete_draft_invoice(draft, 1))
        self.assertFalse(
            self.sales.db.execute_query(
                "SELECT 1 FROM invoices WHERE invoice_number=?", (draft,)
            )
        )
