...
def main():
    ...
    try:
        ...
        with db.transaction() as conn:
            conn.executemany(
                "INSERT INTO products(part_no,description,brand,cost_price,selling_price,quantity_on_hand,active) VALUES(?,?,?,?,?,?,?)",
                rows,
            )
        insert_seconds = time.perf_counter() - started
        started = time.perf_counter()
        result = db.execute_query(
            "SELECT id, part_no FROM products WHERE part_no=? AND active=1",
            ("BENCH-099999",),
        )
        exact_ms = (time.perf_counter() - started) * 1000
        started = time.perf_counter()
        matches = db.execute_query(
            "SELECT id FROM products WHERE description LIKE ? AND active=1 LIMIT 50",
            ("%Brake Pad 999%",),
        )
        contains_ms = (time.perf_counter() - started) * 1000
        if not result or not matches:
            raise RuntimeError(
                "Benchmark search did not return expected products"
            )
