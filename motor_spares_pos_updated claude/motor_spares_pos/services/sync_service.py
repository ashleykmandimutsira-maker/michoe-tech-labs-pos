...
class SyncService:
    ...
    def endpoint(self) -> str:
        ...
        row = self.db.execute_query("SELECT value FROM settings WHERE key = 'sync_endpoint' LIMIT 1")
        configured = row[0]["value"] if row else ""
        return (configured or self.config.get("server_url") or "").strip().rstrip("/")

    @property
    def timeout(self) -> float:
        try:
            return max(2.0, min(float(self.config.get("sync_timeout", 5)), 15.0))
    ...
    def enqueue(
        ...
        self.db.execute_update(query, (entity_type, entity_id, operation, payload))
        ...
        logger.info("Queued for sync: %s#%s (%s) -> queue %s", entity_type, entity_id, operation, queue_id)
        ...
        emit_change(entity_type, {"entity_id": entity_id, "operation": operation})
        return queue_id
    ...
    def get_sync_status(self) -> dict:
        rows = self.db.execute_query("SELECT status, COUNT(*) count FROM sync_queue GROUP BY status")
    ...
    def request_background_sync(self) -> None:
        ...
        if os.environ.get("POS_DISABLE_BACKGROUND_SYNC") == "1":
            return
        if not self.server_configured() or not _SYNC_LOCK.acquire(blocking=False):
            return
        ...
        threading.Thread(target=worker, name='POS-BackgroundSync', daemon=True).start()
    ...
    def sync_now(self, max_retries: int = 3, _lock_held: bool = False) -> dict:
        ...
        if not _lock_held:
            ...
            if not acquired_here:
                return {
                    'status': 'BUSY', 'uploaded': 0, 'failed': 0, 'downloaded': 0,
    ...
    def _sync_now_impl(self, max_retries: int = 3) -> dict:
        ...
        for row in self.get_pending():
            ...
            try:
                ...
                if not accepted:
                    raise RuntimeError("Server did not acknowledge the sync record")
                external_id = str(accepted[0].get("external_id") or record["client_key"])
        ...
        if uploaded or downloaded:
            ...
            emit_change("SYNC", {"uploaded": uploaded, "downloaded": downloaded})
        ...
        return {
            ...
            "message": "Synchronization completed." if failed == 0 else "Synchronization completed with failures.",
        }

    def _request_json(self, method: str, url: str, data: Optional[dict] = None) -> dict:
        body = None
    ...
    def _queue_record(self, row: dict) -> dict:
        payload = row["payload"]
        if not payload:
            payload = json.dumps(self._build_payload(row["entity_type"], row["entity_id"], row["operation"]))
        return {
    ...
    def _build_payload(self, entity_type: str, entity_id: int, operation: str) -> dict:
        if entity_type == "PRODUCT":
            row = self.db.execute_query("SELECT * FROM products WHERE id=?", (entity_id,))
            return dict(row[0]) if row else {"id": entity_id, "operation": operation}
        if entity_type == "STOCK_MOVEMENT":
            row = self.db.execute_query("SELECT * FROM stock_movements WHERE id=?", (entity_id,))
            return dict(row[0]) if row else {"id": entity_id, "operation": operation}
        if entity_type == "SALE":
            sale = self.db.execute_query("SELECT * FROM sales WHERE id=?", (entity_id,))
            if not sale:
                return {"id": entity_id, "operation": operation}
            result = dict(sale[0])
            result["items"] = [dict(r) for r in self.db.execute_query("SELECT * FROM sale_items WHERE sale_id=? ORDER BY id", (entity_id,))]
            result["payments"] = [dict(r) for r in self.db.execute_query("SELECT * FROM payments WHERE sale_id=? ORDER BY id", (entity_id,))]
            return result
        if entity_type == "RETURN":
            ret = self.db.execute_query("SELECT * FROM returns WHERE id=?", (entity_id,))
            result = dict(ret[0]) if ret else {"id": entity_id, "operation": operation}
            result["items"] = [dict(r) for r in self.db.execute_query("SELECT * FROM return_items WHERE return_id=? ORDER BY id", (entity_id,))]
            return result
        if entity_type == "REFUND":
            row = self.db.execute_query("SELECT * FROM refunds WHERE id=?", (entity_id,))
            return dict(row[0]) if row else {"id": entity_id, "operation": operation}
        if entity_type == "CUSTOMER":
            row = self.db.execute_query("SELECT * FROM customers WHERE id=?", (entity_id,))
            return dict(row[0]) if row else {"id": entity_id, "operation": operation}
        if entity_type == "VEHICLE":
            row = self.db.execute_query("SELECT * FROM vehicles WHERE id=?", (entity_id,))
            return dict(row[0]) if row else {"id": entity_id, "operation": operation}
        return {"id": entity_id, "operation": operation}

    def _pull_updates(self) -> int:
        row = self.db.execute_query("SELECT value FROM settings WHERE key='sync_last_pulled_at' LIMIT 1")
        since = row[0]["value"] if row else ""
        query = urlencode({"shop_id": self.shop_id, "since": since})
        response = self._request_json("GET", f"{self.endpoint}/api/sync/pull?{query}")
    ...
    def _apply_remote_product_delete(self, remote: dict) -> bool:
        ...
        if not part_no:
            return False
        rows = self.db.execute_query("SELECT id FROM products WHERE part_no=?", (part_no,))
        if rows:
            self.db.execute_update("UPDATE products SET active=0, updated_at=CURRENT_TIMESTAMP WHERE id=?", (rows[0]["id"],))
        return True
    ...
    def _apply_remote_customer(self, remote: dict) -> bool:
        ...
        if self._has_pending_local_change("CUSTOMER", customer_id):
            return False
        fields = ["customer_code", "name", "company", "phone", "email", "address", "city", "country", "id_number",
                  "vehicle_make", "vehicle_model", "vehicle_registration"]
        values = [remote.get(field) for field in fields]
        existing = self.db.execute_query("SELECT id FROM customers WHERE id=?", (customer_id,))
        if existing:
            self.db.execute_update(
                "UPDATE customers SET " + ", ".join(f"{field}=?" for field in fields) + ", active=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
                tuple(values) + (1 if remote.get("active", True) else 0, customer_id),
            )
        else:
            self.db.execute_update(
                "INSERT INTO customers (id," + ",".join(fields) + ",active) VALUES (?" + ",?" * len(fields) + ",?)",
                (customer_id,) + tuple(values) + (1 if remote.get("active", True) else 0,),
            )
        return True
    ...
    def _apply_remote_vehicle(self, remote: dict) -> bool:
        ...
        if vehicle_id is None or not registration or customer_id is None or self._has_pending_local_change("VEHICLE", vehicle_id):
            return False
        if not self.db.execute_query("SELECT id FROM customers WHERE id=?", (customer_id,)):
            return False
        fields = ["customer_id", "registration_number", "make", "model", "year", "engine", "vin", "color", "notes"]
        values = [remote.get(field) for field in fields]
        existing = self.db.execute_query("SELECT id FROM vehicles WHERE id=?", (vehicle_id,))
        if existing:
            self.db.execute_update("UPDATE vehicles SET " + ", ".join(f"{field}=?" for field in fields) + ",active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?", tuple(values) + (1 if remote.get("active", True) else 0, vehicle_id))
        else:
            self.db.execute_update("INSERT INTO vehicles(id," + ",".join(fields) + ",active) VALUES (?" + ",?" * len(fields) + ",?)", (vehicle_id,) + tuple(values) + (1 if remote.get("active", True) else 0,))
        return True

    def _has_pending_local_change(self, entity_type: str, entity_id: Optional[int]) -> bool:
        if entity_id is None:
            return False
        return bool(self.db.execute_query("SELECT 1 FROM sync_queue WHERE entity_type=? AND entity_id=? AND operation IN ('CREATE','UPDATE') AND status IN ('PENDING','SYNCING','FAILED') LIMIT 1", (entity_type, entity_id)))

    def _has_pending_local_product_change(self, product_id: Optional[int]) -> bool:
    ...
    def _apply_remote_product(self, remote: dict) -> bool:
        ...
        if not part_no:
            return False
        if self.db.execute_query("SELECT 1 FROM product_tombstones WHERE part_no=? LIMIT 1", (part_no,)):
            return False

        existing = self.db.execute_query("SELECT * FROM products WHERE part_no=? LIMIT 1", (part_no,))
        if existing:
            product_id = existing[0]["id"]
            if self._has_pending_local_product_change(product_id):
                logger.info("Skipping remote product update for %s because local changes are pending", part_no)
                return False
            ...
            if remote_updated and local_updated and remote_updated <= local_updated:
                return False
            fields = [
                "barcode", "description", "brand", "category_id", "vehicle_make", "vehicle_model",
                "cost_price", "selling_price", "currency", "quantity_on_hand", "reorder_level", "vat_rate", "active"
            ]
            values = [remote.get(field) for field in fields]
            self.db.execute_update(
                "UPDATE products SET " + ", ".join(f"{field}=?" for field in fields) + ", updated_at=CURRENT_TIMESTAMP WHERE id=?",
        ...
        try:
            self.db.execute_update(
                ...
                (
                    remote.get("barcode"), part_no, remote.get("description") or part_no, remote.get("brand") or "",
                    remote.get("category_id"), remote.get("vehicle_make"), remote.get("vehicle_model"),
                    float(remote.get("cost_price") or 0), float(remote.get("selling_price") or 0), remote.get("currency") or "USD",
                    int(remote.get("quantity_on_hand") or 0), int(remote.get("reorder_level") or 5), float(remote.get("vat_rate") or 15),
                    1 if remote.get("active", True) else 0,
                ),
            )
            return True
        except Exception as exc:
            logger.warning("Could not apply remote product %s: %s", part_no, exc)
            return False
    ...
    def mark_synced(self, queue_id: int, external_id: str) -> bool:
        try:
            ...
            logger.error("Failed to mark sync row %s as synced: %s", queue_id, exc)
            return False
    ...
    def mark_failed(self, queue_id: int, error_message: str = "") -> bool:
        try:
            ...
            logger.error("Failed to mark sync row %s as failed: %s", queue_id, exc)
            return False
    ...
    def retry_failed(self, max_retries: int = 3) -> int:
        ...
        rows = self.db.execute_query("SELECT COUNT(*) count FROM sync_queue WHERE status='PENDING'", ())
        return int(rows[0]["count"]) if rows else 0

    def log_sync(self, queue_id: int, success: bool, server_response: Optional[str] = None) -> bool:
        row = self.db.execute_query(
            "SELECT entity_type,entity_id,operation FROM sync_queue WHERE id=?", (queue_id,)
        )
        if not row:
            return False
        try:
            self.db.execute_update(
                ...
                (
                    row[0]["entity_type"], row[0]["entity_id"], row[0]["operation"],
                    "SUCCESS" if success else "FAILED", None, queue_id, 1 if success else 0, server_response,
                ),
            )
            return True
        except Exception as exc:
