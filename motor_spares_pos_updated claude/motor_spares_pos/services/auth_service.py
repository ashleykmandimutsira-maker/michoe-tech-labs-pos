...
class AuthenticationService:
    ...
    def __init__(self):
        ...
        if bcrypt is None:
            logger.warning(
                "bcrypt not installed - password hashing unavailable"
            )
    ...
    def change_user_role(
        self, admin_user_id: int, target_user_id: int, role_id: int
    ) -> bool:
        ...
        role = self.db.execute_query(
            "SELECT id, name FROM roles WHERE id = ?", (role_id,)
        )
        if not admin or not admin.is_active or not admin.is_admin():
            raise PermissionError(
                "Only an active ADMIN can change user roles."
            )
        if not self._db_user_has_permission(
            admin_user_id, "MANAGE_PERMISSIONS"
        ):
            raise PermissionError(
                "Administrator does not have MANAGE_PERMISSIONS."
            )
        if not target:
            raise ValueError("Target user was not found.")
        if admin_user_id == target_user_id:
            raise ValueError(
                "Do not change the currently logged-in administrator's role."
            )
        if not role:
        ...
        logger.info(
            "Changed role for user %s to %s by admin %s",
            target_user_id,
            role[0]["name"],
            admin_user_id,
        )
    ...
    def logout(self, user_id: int) -> None:
        ...
        AuditService().log_action(
            "LOGOUT", "USER", user_id, user_id, details="User session ended"
        )
    ...
    def delete_user(
        self, admin_user_id: int, target_user_id: int, reason: str
    ) -> bool:
        ...
        if not admin or not admin.is_active or not admin.is_admin():
            raise PermissionError(
                "Only an active administrator can delete users."
            )
        ...
        AuditService().log_action(
            ...
            admin_user_id,
            reason=reason.strip(),
            details=f"Access disabled for {target.username}; historical records preserved.",
        )
        logger.info(
            "User access deleted: %s by administrator %s",
            target.username,
            admin_user_id,
        )
        return True
    ...
    def set_user_permissions(
        ...
        if not admin or not admin.is_active or not admin.is_admin():
            raise PermissionError(
                "Only an active ADMIN can manage user privileges."
            )
        if not self._db_user_has_permission(
            admin_user_id, "MANAGE_PERMISSIONS"
        ):
        ...
        if admin_user_id == target_user_id:
            raise ValueError(
                "Do not remove or replace the currently logged-in administrator's privileges."
            )
        ...
        if invalid:
            raise ValueError(
                "Unknown permissions: " + ", ".join(sorted(invalid))
            )
        ...
        for code in sorted(requested - set(current)):
            ...
            AuditService().log_permission_grant(
                target_user_id, known[code], admin_user_id, code
            )
            changed["granted"].append(code)

        for code in sorted(set(current) - requested):
            if (
                code in ("DELETE_PRODUCT", "RESTORE_PRODUCT")
                and target.role
                and target.role.name != "ADMIN"
            ):
                continue
