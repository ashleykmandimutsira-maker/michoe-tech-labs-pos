...
HEADER_FILL = PatternFill(
    start_color="1F2933", end_color="1F2933", fill_type="solid"
)
...
class SpreadsheetService:
    ...
    def export_inventory(
        self, path: str, include_cost_price: bool = True
    ) -> int:
        ...
        category_names = {
            c.id: c.name for c in self.products.get_all_categories()
        }
        logger.info(
            "DATABASE QUERY COMPLETE elapsed=%.3fs",
            time.perf_counter() - query_started,
        )
        columns = (
            COLUMNS
            if include_cost_price
            else [c for c in COLUMNS if c[0] != "cost_price"]
        )
        ...
        for row_idx, product in enumerate(items, start=2):
            ...
            for col_idx, (key, _label, _required) in enumerate(
                columns, start=1
            ):
        ...
        logger.info(
            "XLSX GENERATION COMPLETE elapsed=%.3fs",
            time.perf_counter() - started,
        )
        wb.save(path)
        logger.info(
            "FILE WRITE COMPLETE elapsed=%.3fs", time.perf_counter() - started
        )
    ...
    def _write_header(ws, columns) -> None:
        for col_idx, (_key, label, required) in enumerate(columns, start=1):
            cell = ws.cell(
                row=1, column=col_idx, value=label + (" *" if required else "")
            )
    ...
    def import_inventory(
        self, path: str, user_id: Optional[int] = None
    ) -> ImportResult:
        ...
        """
        result = ImportResult(
            filename=str(path), import_batch_id=str(uuid.uuid4())
        )
        ...
        missing = [
            label
            for key, label, required in COLUMNS
            if required and key not in col_index
        ]
        if missing:
            raise ValueError(
        ...
        with batch_changes("INVENTORY_IMPORT"):
            for row_num, row in enumerate(ws.iter_rows(min_row=2), start=2):
                values = {
                    key: self._cell(row, idx) for key, idx in col_index.items()
                }
                ...
                if not part_no and not description:
                    continue
                try:
                    before = self._snapshot_by_identity(
                        part_no, values.get("barcode")
                    )
                    self._apply_row(
                        row_num,
                        values,
                    ...
                    after = self._snapshot_by_identity(
                        part_no, values.get("barcode")
                    )
                    ...
                    result.errors.append(
                        f"Row {row_num} ({part_no or '?'}): {exc}"
                    )

        status = (
            "FAILED"
            if result.errors and result.created == 0 and result.updated == 0
            else "PARTIAL" if result.errors else "COMPLETED"
        )
        self.db.execute_update(
    ...
    def list_imports(self, limit=50):
        return self.db.execute_query(
            "SELECT * FROM inventory_imports ORDER BY imported_at DESC LIMIT ?",
            (limit,),
        )
    ...
    def undo_import(self, import_id: int, admin_user_id: int) -> dict:
        ...
        batch = self.db.execute_query(
            "SELECT * FROM inventory_imports WHERE id=?", (import_id,)
        )
        if not batch:
        ...
        items = self.db.execute_query(
            "SELECT * FROM inventory_import_items WHERE import_id=? ORDER BY id DESC",
            (import_id,),
        )
        removed = 0
        restored = 0
            ...
            for item in items:
                if item["action"] == "CREATED" and item["product_id"]:
                    dependent = conn.execute(
                        "SELECT 1 FROM sale_items WHERE product_id=? UNION SELECT 1 FROM stock_movements WHERE product_id=? UNION SELECT 1 FROM return_items WHERE product_id=? LIMIT 1",
                        (
                            item["product_id"],
                            item["product_id"],
                            item["product_id"],
                    ...
                    if not dependent:
                        conn.execute(
                            "UPDATE inventory_import_items SET product_id=NULL WHERE id=?",
                            (item["id"],),
                        )
                        conn.execute(
                        ...
                        conn.execute(
                            ...
                            (item["product_id"], item["part_no"]),
                        )
                        removed += 1
                        continue
                before = (
                    json.loads(item["before_values"])
                    if item["before_values"]
                    else None
                )
                if before and item["product_id"]:
                    conn.execute(
                        "UPDATE products SET barcode=?,oem_number=?,part_no=?,description=?,brand=?,category_id=?,vehicle_make=?,vehicle_model=?,vehicle_year_from=?,vehicle_year_to=?,cost_price=?,selling_price=?,currency=?,quantity_on_hand=?,reorder_level=?,vat_rate=?,active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",
                        (
                            before.get("barcode"),
                            before.get("oem_number"),
                            before.get("part_no"),
                            before.get("description"),
                            before.get("brand"),
                            before.get("category_id"),
                            before.get("vehicle_make"),
                            before.get("vehicle_model"),
                            before.get("vehicle_year_from"),
                            before.get("vehicle_year_to"),
                            before.get("cost_price"),
                            before.get("selling_price"),
                            before.get("currency"),
                            before.get("quantity_on_hand"),
                            before.get("reorder_level"),
                            before.get("vat_rate"),
                            before.get("active"),
                            item["product_id"],
                        ),
                    ...
                    restored += 1
            conn.execute(
                "UPDATE inventory_imports SET status='UNDONE' WHERE id=?",
                (import_id,),
            )
            if trigger:
    ...
    def _apply_row(
        ...
        if category_name:
            ...
            if category_id is None:
                category_id = self.products.create_category(
        ...
        reorder_level = self._num(
            values.get("reorder_level"), default=None, integer=True
        )
        target_quantity = self._num(
            values.get("quantity_on_hand"), default=None, integer=True
        )
        ...
        if existing:
            ...
            updated = Product(
                ...
                part_no=part_no,
                oem_number=str(
                    values.get("oem_number") or existing.oem_number or ""
                )
                or None,
                ...
                vehicle_make=str(
                    values.get("vehicle_make") or existing.vehicle_make or ""
                )
                or None,
                vehicle_model=str(
                    values.get("vehicle_model") or existing.vehicle_model or ""
                )
                or None,
                vehicle_year_from=self._num(
                    values.get("vehicle_year_from"),
                    default=existing.vehicle_year_from,
                    integer=True,
                ),
                vehicle_year_to=self._num(
                    values.get("vehicle_year_to"),
                    default=existing.vehicle_year_to,
                    integer=True,
                ),
                cost_price=(
                    cost_price
            ...
            if any(
                getattr(updated, f) != getattr(existing, f)
            ...
            if (
                ...
                and target_quantity != existing.quantity_on_hand
            ):
            ...
            if not part_no or not description:
                raise ValueError(
                    "New products need at least a Part No and Description"
                )
            new_product = Product(
                barcode=barcode,
                part_no=part_no,
                ...
                vehicle_year_from=self._num(
                    values.get("vehicle_year_from"), default=None, integer=True
                ),
                vehicle_year_to=self._num(
                    values.get("vehicle_year_to"), default=None, integer=True
                ),
                cost_price=cost_price or 0.0,
                selling_price=selling_price or 0.0,
                quantity_on_hand=target_quantity or 0,
                reorder_level=(
                    reorder_level if reorder_level is not None else 5
                ),
            )
