...
def test_add_different_products():
    ...
    logger.info(
        f"✓ Product 1: {spark_plugs.part_no} - {spark_plugs.description} (${spark_plugs.selling_price})"
    )
    logger.info(
        f"✓ Product 2: {oil_filter.part_no} - {oil_filter.description} (${oil_filter.selling_price})"
    )
    ...
    # Verify independence
    assert (
        spark_in_cart["part_no"] == spark_plugs.part_no
    ), "Spark plug part_no mismatch"
    assert (
        spark_in_cart["description"] == spark_plugs.description
    ), "Spark plug description mismatch"
    assert (
        spark_in_cart["brand"] == spark_plugs.brand
    ), "Spark plug brand mismatch"
    assert (
        spark_in_cart["unit_price"] == spark_plugs.selling_price
    ), "Spark plug price mismatch"

    assert (
        oil_in_cart["part_no"] == oil_filter.part_no
    ), "Oil filter part_no mismatch"
    assert (
        oil_in_cart["description"] == oil_filter.description
    ), "Oil filter description mismatch"
    assert (
        oil_in_cart["brand"] == oil_filter.brand or ""
    ), "Oil filter brand mismatch"
    assert (
        oil_in_cart["unit_price"] == oil_filter.selling_price
    ), "Oil filter price mismatch"

    # THE CRITICAL TEST: Verify they don't share data
    assert (
        spark_in_cart["brand"] != oil_in_cart["brand"]
    ), f"FAIL: Brands match! {spark_in_cart['brand']} == {oil_in_cart['brand']}"
    assert (
        spark_in_cart["description"] != oil_in_cart["description"]
    ), "FAIL: Descriptions match!"
    assert (
        spark_in_cart["unit_price"] != oil_in_cart["unit_price"]
    ), "FAIL: Prices match!"

    logger.info(f"\n✓ PASS: Items are independent")
    logger.info(
        f"  Spark brand ({spark_in_cart['brand']}) != Oil brand ({oil_in_cart['brand']})"
    )
    logger.info(
        f"  Spark price (${spark_in_cart['unit_price']:.2f}) != Oil price (${oil_in_cart['unit_price']:.2f})"
    )
...
def test_same_product_increases_quantity():
    ...
    logger.info(
        f"✓ Product: {spark_plugs.part_no} - {spark_plugs.description}"
    )
    ...
    assert (
        cart_item_after_first["quantity"] == 1
    ), "First add should set quantity to 1"
    logger.info(
    ...
    assert (
        cart_item_after_second["quantity"] == 2
    ), "Second add should increment quantity to 2"
    ...
    # Verify line total
    line_total = (
        cart_item_after_second["unit_price"]
        * cart_item_after_second["quantity"]
    )
    expected_total = spark_plugs.selling_price * 2
    assert (
        line_total == expected_total
    ), f"Line total mismatch: {line_total} != {expected_total}"
    logger.info(
...
def test_cart_total_calculation():
    ...
    for product_id, item in cart.cart.items():
        ...
        logger.info(
            f"  {item['part_no']}: {item['quantity']} × ${item['unit_price']:.2f} = ${line:.2f}"
        )
    ...
    assert (
        abs(cart_total - expected_total) < 0.01
    ), f"Total mismatch: {cart_total} != {expected_total}"
...
def test_product_independence_after_quantity_change():
    ...
    assert (
        spark_after["quantity"] == spark_initial["quantity"]
    ), "Quantity changed!"
    assert spark_after["brand"] == spark_initial["brand"], "Brand changed!"
    assert (
        spark_after["unit_price"] == spark_initial["unit_price"]
    ), "Price changed!"
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("\nThe cart now:")
        logger.info(
            "  - Stores independent data for each product (no shared references)"
        )
